...
function inferCustomReason(row) {
  ...
  if (/unarmed/.test(text)) return 'unarmed';
  if (/claw|bite|gore|slam|sting|tail|horn|natural/.test(text)) return 'natural';
  if (/self-destruct|self destruct|pacify hostile|poison|disease|condition-track|condition track/.test(text)) return 'special/rider';
  if (/grenade|missile|rocket|flame|flamethrower|blast|burst|cone|line|area|splash|autofire/.test(text)) return 'area/special-mode';
  return null;
}
