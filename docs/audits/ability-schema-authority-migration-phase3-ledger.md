...
|---|---|---|---|
| 1 | `scripts/combat/rolls/enhanced-rolls.js:1482` (`SWSERoll.rollInitiative()`) | **Live initiative rolls.** `actor.system.abilities?.dex?.mod ?? 0` — no fallback at all. | None |
| 2 | `scripts/ui/combat-stats-tooltip.js:175` (`getInitiativeBreakdown()`) | Initiative breakdown tooltip display. | None |
