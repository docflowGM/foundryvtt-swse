...
Fixed and certified:
1. Grapple: **SSOT-certified AND rules-fidelity-certified**. `computeGrappleBonus()` (`combat-stat-rules.js`) is now the only grapple arithmetic in the codebase, and its `GRAPPLE_SIZE_MODIFIERS` table has been checked against the published Core Rulebook table (and cross-validated against real Aiwha/Bantha compendium stat blocks) after an earlier centralization pass turned out to have quietly encoded a wrong, step-of-4 table. Six call sites now provably cannot disagree because they call the same function: `derived-calculator.js`, `resolveGrappleBonus(actor)`, `houserule-grapple.js`'s roll formula, `PanelContextBuilder`'s displayed sheet box, `combat-stats-tooltip.js`'s hover breakdown, and `SWSEGrappling._rollGrappleBonus()` (the base for every opposed Grab/Grapple/Pin/Trip/Throw/Crush/Escape check). `getGrappleDC()`'s BAB read also corrected to `SchemaAdapters.getBAB()`. Along the way this also fixed: a previously-invisible bug where `system.derived.grappleBonus` was always `NaN` (masked by a sheet-side fallback guard); a sheet-display bug where a legitimate canonical `0` could be silently discarded in favor of a private fallback formula; an extra half-heroic-level term the opposed-combat-check engine added that the published rule does not have; and a BAB-box display-priority conflict where the sheet's editable BAB field could show a stale value instead of the canonical computed one. See the Grapple domain section for the full three-round history.
...
## Domain: Grapple — CONFIRMED DEFECT (roll-time formula diverges from displayed formula) — **FIXED, SSOT-CERTIFIED, RULES-FIDELITY-CERTIFIED**
...
- `scripts/data/background-events.js:96` (the "Enslaved" background's "Grapple Survivor" special ability, `target:'grapple', value:2, requiresRuntime:false`) — **newly discovered, deeper gap, flagged for a future audit, not fixed here**: tracing where `target:'grapple'` modifiers are supposed to be consumed found that `system.derived.modifiers` (`derived-calculator.js:1000`) is shaped as `{all, breakdown}`, not keyed by target name — so `combat-stats-tooltip.js`'s pre-existing `_getModifiersForTarget(actor, 'grapple')` (used for contextual-modifier display rows) has always returned `[]` regardless of any background-granted grapple bonus, and no other grapple resolver in the codebase reads a target-keyed modifier list either. Whether this specific background bonus is applied anywhere at all is unresolved and needs its own audit — it is a modifier-pipeline wiring question, not a grapple-formula question, and touching it is out of scope for this pass.
