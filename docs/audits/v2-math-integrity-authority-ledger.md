...
Fixed and certified:
1. Grapple: **SSOT-certified**. `computeGrappleBonus()` (`combat-stat-rules.js`) is now the only grapple arithmetic in the codebase; `derived-calculator.js` and `resolveGrappleBonus(actor)` both delegate to it, and `houserule-grapple.js`'s roll formula reads the resulting canonical value with that same resolver as its only fallback. `getGrappleDC()`'s BAB read also corrected to `SchemaAdapters.getBAB()`. Bonus discovery: this extraction fixed an independent, previously-invisible bug where `system.derived.grappleBonus` was always `NaN` (masked by a sheet-side fallback guard) — see the Grapple domain section.
...
## Domain: Grapple — CONFIRMED DEFECT (roll-time formula diverges from displayed formula) — **FIXED, SSOT-CERTIFIED**
