...
**Correction to prior rounds' wording (round 7):** "New Faction id generation: `randomId()`" and "New Contact id generation: `randomId()`" above described round 1's shipped behavior accurately at the time, but round 7 below upgraded ordinary creation to mint through the structurally collision-safe `mintUniqueId()` (the same helper migration already used) instead of plain `randomId()`. This section is left as an unedited historical record of what shipped in round 1; round 7's own correction-pass section and the Final verdict are authoritative for the current contract.
...
### Correction pass (independent review of PR #969)

Independent review of the pushed branch agreed with the core identity policy (id-only mutation, name-independent new-id minting, legacy-id preservation, duplicate names legal, Contact→Actor separation) but found the hardening had stopped one layer too early: `upsertFaction()`/`upsertFactionContact()` themselves were fixed, but several OTHER canonical mutation paths still resolved their target Faction/Contact through the intentionally flexible, name-capable `findFaction()`/`findFactionContact()` **search** helpers — unsafe now that duplicate display names are explicitly legal, since a name match could still silently resolve to the *first* of several same-named candidates. Ten concrete issues, all fixed on the same branch/PR, none requiring architecture changes:

1. **Canonical Contact mutators still resolved their parent Faction by name.** `upsertFactionContact()`'s Faction lookup (`data.factionName` fallback), `deleteFactionContact()`, and `promoteFactionContactToActor()` (both its Faction *and* its Contact resolution, the latter via `findFactionContact()`'s own name-matching) all used the flexible search helpers to decide which record to mutate. Fixed: four new private resolvers — `_resolveFactionByUniqueName()`, `_resolveFactionForMutation(id, name)` (STRICT: a supplied id is always authoritative, never abandoned for a name match, even an id that fails to resolve; only when no id at all is given does an UNAMBIGUOUS name get tried), `_resolveFactionByIdOrUniqueName(idOrName)` (PERMISSIVE single-string form for the handful of UI call sites that only ever carry one combined `factionId || factionName`-style value — tries the string as an exact id first, then as a name), and `_resolveFactionContactByIdOrUniqueName(faction, idOrName)`. All four return `{ record, ambiguous }`; `ambiguous: true` (2+ same-named candidates) is never silently resolved to the first match. `upsertFactionContact()`/`deleteFactionContact()`/`promoteFactionContactToActor()` now all resolve through these and throw a clear, named error on ambiguity rather than guessing. `findFaction()`/`findFactionContact()` themselves are untouched — still the intentional, flexible search helpers for read/display use (§4).
2. **Actor↔Faction mutation still merged by display name.** `addActorRelationship()`'s Faction resolution (`factionId ? findFaction(factionId) : null) || (factionName ? findFaction(factionName) : null)`) and its "does this Actor already have a relationship with this Faction" lookup (`entry.factionId === factionRecord.id || entry.factionName.toLowerCase() === factionRecord.name.toLowerCase()`) both used name equality; `updateActorRelationship()`'s Faction resolution did the same; `applyScoreDelta()`'s Faction resolution and its own existing-relationship lookup did too. Fixed: all four now resolve the Faction via `_resolveFactionForMutation(id, name)` (STRICT — id always wins when supplied); the "does this Actor already have this relationship" lookups now match by `factionId` only (never a name-equality fallback — `factionRecord.id` is always a real, already-disambiguated id by that point, so a name-equality fallback there could only ever update the WRONG same-named Faction's relationship row). `addActorRelationship()`/`updateActorRelationship()` throw on ambiguity (consistent with the file's existing throw-on-invalid-input convention). `applyScoreDelta()` deliberately fails **soft** instead (returns `null`, logs an `SWSELogger.warn`) — it runs in a per-actor loop from `applyJobFactionDelta()`, and a thrown error there would abort every OTHER actor's unrelated Job-consequence update in the same batch.
3. **`approveSuggestedFaction()`'s own id-or-name resolution (added in the first pass specifically to preserve "attach to a known same-named Faction" behavior) picked the first same-named Faction on ambiguity.** Fixed: now uses `_resolveFactionForMutation()` and throws a clear error telling the GM to resolve which Faction the suggestion refers to (e.g. via the Faction editor) before approving, rather than silently attaching to whichever `findFaction()` happened to return first.
4. **`GMJobBoardSurfaceController#_saveClientAsContact()`** (the Job Board's "save client as reusable contact" free-text flow — genuinely has no id widget, only a `clientFaction` text field) used `findFaction(factionLabel) || upsertFaction(...)`, silently picking the first same-named Faction. Fixed: added a new **public** `FactionRegistryService.resolveOrCreateFactionByName(name, createData)` — the external-caller counterpart to the private resolvers: reuses an existing Faction only when the name is unambiguous, creates new on zero matches, throws on 2+. Also fixed a second, independent bug the review caught: since `upsertFactionContact()` was hardened to match an existing Contact by id only, and this caller never passed one, repeatedly clicking "save as reusable contact" for the same client name would now spawn a new duplicate Contact on every save instead of updating the existing reusable profile (the pre-hardening behavior, which relied on Contact-side name matching). Fixed at the caller: look up any existing same-name Contact on the resolved Faction via `getFactionContacts()`, and pass its id through only when there is exactly one match (never on 0 or 2+, so it can never merge into the wrong one or silently pick among several).
5. **`GMLocationsSurfaceController`'s Atlas lead reveal** resolved `revealFactionIds` via `findFaction()` (name-capable) and `revealContactIds` via `allContacts.find(c => c.id === contactId || c.name === contactId)` — despite both fields being literally named "...Ids" and the UI itself labeling the field "Reveal Contact ids on success." Fixed: both now resolve by exact id only (`registry.find(entry => entry.id === factionId)` / `allContacts.find(contact => contact.id === contactId)`), with the display-name fallback removed entirely — no Location architecture was touched, this only tightens the identity contract the field name already promised.
6. **`migrateLegacyIdentities()` only detected duplicate Contact ids within a single Faction** (`seenContactIds` was a fresh `Set` reset on every Faction iteration), but old name/role-derived Contact ids could plausibly collide **across** two different Factions, and every id-only Contact consumer (`getAllFactionContacts()`, and now the Atlas lead reveal fix above) addresses a Contact by its id alone, registry-wide, not scoped to a Faction. Fixed: Contact id claims are now tracked in ONE `Set` global across the entire migration sweep, so a cross-Faction duplicate is caught and disambiguated exactly like a same-Faction one.
7. **Migration id minting was not structurally collision-safe.** The original pass minted a fresh id inline per missing/duplicate record as it was encountered, adding each to a `Set` grown incrementally — so a record processed early that needed a fresh id could, in a vanishingly unlikely but structurally possible case, draw the exact id a **healthy** record later in the registry already legitimately held, forcing that later healthy record to be misidentified as the duplicate and rewritten (violating the migration's own "a healthy legacy id is never touched" guarantee). Fixed: every pre-existing, non-empty Faction id and Contact id in the raw registry is now collected into a reserved-id `Set` in a separate pass **before** any minting begins; a new `mintUniqueId(reserved)` helper (`randomId()`, retried until the draw isn't in `reserved`, then added to it) guarantees a freshly-minted id can never collide with ANY pre-existing id, regardless of processing order. Verified with a forced-collision test (the `randomID` stub is made to draw a healthy record's exact existing id on its first call) — the healthy record's id survives unchanged and the minter retries.
8. **A genuine duplicate persisted id is intrinsically ambiguous for any *existing* external reference** (a Job/Intel/Location record that already pointed at that id before migration ran) — there is no way to know which of the two records it meant, and the original pass's audit prose claimed (inaccurately) that no such ambiguous case existed. Fixed: `migrateLegacyIdentities()` now returns a `collisions` array (`{ kind, duplicateId, reassignedTo, factionId?, name }` per genuine duplicate — as opposed to a merely-missing id, which is not a collision) and emits one `SWSELogger.warn` per collision naming the duplicated id, which record kept it, which was reassigned, and that any pre-existing external reference to the duplicated id cannot be automatically disambiguated. The migration still never guesses or fuzzy-repairs a reference — it reports the ambiguity rather than pretending it doesn't exist.
9. **Wording**: the original PR body/test file described migrated ids as assigned "deterministically." They are not deterministic from identical raw input (they come from `randomId()`); corrected throughout to "minted exactly once, persisted, and thereafter stable across reads, with the migration itself idempotent."
10. **Test coverage** was expanded from the original 29-item matrix (23 assertion-groups) to 34 assertion-groups: 11 new tests (30-40) directly exercise every path items 1-8 above touch — `addActorRelationship`/`updateActorRelationship`/`applyScoreDelta` with duplicate same-named Factions (including proving a PC can legitimately hold distinct relationships with two same-named Factions, and that `applyScoreDelta` fails soft rather than crashing a batch), `approveSuggestedFaction` and `resolveOrCreateFactionByName` ambiguity, `upsertFactionContact`/`deleteFactionContact`/`promoteFactionContactToActor` refusing to guess among duplicate Factions *and* duplicate Contacts, a cross-Faction duplicate-Contact-id migration test, a forced-mint-collision test proving healthy ids survive, and structural regression guards (source-pattern assertions) plus a functional algorithm replica tying the two UI-controller fixes (items 4 and 5) back to the real caller files without pulling their full render/host dependency graphs into this identity-focused test file. Test #29's own comment was corrected to state accurately what it proves (the two shared upserts specifically) rather than the broader claim now proven by 30-40 collectively.

None of these ten fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic identity/migration framework — `resolveOrCreateFactionByName()` and the four private resolvers are small, narrowly-scoped, domain-specific seams, not a reusable entity-resolution engine.

Re-ran full validation after the correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 34/34 assertion-groups pass; `gm-faction-ecosystem-view-model`/`gm-job-ecosystem-view-model`/`gm-locations-ecosystem-view-model`/`gm-intel-ecosystem-view-model`/`gm-generation-phase8d3b/8d3c/8d1`/`gm-datapad-no-duplicate-handler-regression`/`gm-campaign-context-parity`/`gm-campaign-context-read-only-contract`/`gm-faction-context-navigation-controller`/`gm-workspace-faction-contact-navigation-controller`/`gm-holonet-phase8b/8c` all unaffected; `node tools/run-rolling-tests.mjs`, `node tools/run-rolling-syntax-check.mjs`, `node tools/check-architecture-boundaries.mjs`, `node tools/validate-partials.mjs`, `node tools/validate-data.js`, and `system.json` parse were all re-run (see the Validators section above, updated in place with the post-correction-pass counts).

### Correction pass round 2 (independent re-review of the round-1 correction pass)

A second independent re-review, against the round-1 correction-pass head, confirmed round 1 fixed nearly everything it raised but found three further identity issues — two significant. All three fixed on the same branch/PR:

1. **Legacy migration could orphan a reference that was valid under the OLD system.** Pre-hardening code read a Faction/Contact record with no persisted `id` as `slugify(name)` / `slugify(name-role)` on every read — a deterministic, if collision-prone, "effective identity." Any code path that resolved such a record's `.id` and copied it into ANOTHER persisted document (a Location's `controllingFactionId`, an Intel record's `linkedFactionId`, a Job's `issuerFactionId`, an Actor relationship's `factionId`) would have captured that exact slugified value. Round 1's fix changed the missing-id fallback (both in `_normalizeFactionRecord()`/`normalizeContact()`'s read-time normalization AND in `migrateLegacyIdentities()`) to mint a fresh `randomId()` instead — correct for duplicate-collision safety, but it meant migrating a real world's pre-existing missing-id record could silently orphan every such external reference, exactly the kind of existing-world breakage this whole phase exists to prevent. Fixed: a missing id now first tries the EXACT legacy formula (`legacyFactionIdCandidate()`/`legacyContactIdCandidate()`, new small module-level helpers reproducing `_normalizeFactionRecord`'s/`normalizeContact`'s original pre-hardening name/role-default logic precisely) as its candidate. If that candidate is not already reserved by a healthy id or claimed by an earlier record in the same pass, it is used directly (`recoveredLegacyIdentity: true` in the returned `migratedFactions`/`migratedContacts` entries) — freezing the record's actual prior effective identity, not merely "some new value." Only when the candidate genuinely collides does it fall back to `mintUniqueId()`, itself reported as a `collisions` entry exactly like a duplicate-persisted-id collision. Read-time normalization (`_normalizeFactionRecord()`/`normalizeContact()`) was reverted to the same `slugify(...)` fallback for the identical reason — this also incidentally fixes round 1's own accepted limitation ("repeated reads of an unmigrated record manufacture a different random id each time"): `slugify()` is a pure function of the name/role, so pre-migration reads are now deterministic and stable again, matching old behavior exactly until migration disambiguates a genuine collision. This is legacy-READ compatibility only — `upsertFaction()`/`upsertFactionContact()` always supply an id explicitly before calling these normalizers, so a genuinely NEW record never reaches this fallback and never gets a name-derived id; only a raw, never-yet-migrated persisted record does. New regression tests 9/10 (rewritten), 9b, 19 (rewritten), and 37 (rewritten to force a legacy-candidate collision, not merely a random-mint collision) cover this.
2. **The Job Board's reusable-contact save (round 1's own fix for a different bug) could still silently merge into an ordinary dossier Contact.** Round 1 added a "reuse an existing Contact if its name uniquely matches" rule to stop `_saveClientAsContact()` from spawning a duplicate Contact on every repeat save — but a bare name match doesn't distinguish a Job Board reusable contact from an ordinary Contact the GM built by hand through the Faction editor; saving a Job client named "Mira" could silently overwrite an unrelated dossier NPC that happened to share that name. Fixed: the reuse lookup now ALSO requires the exact `['job-board', 'reusable-contact']` tags this same method already writes onto every Contact it creates — an ordinary dossier Contact never carries them, so it is never a reuse candidate; a genuine repeat save of the same reusable contact still correctly reuses it. Tests 39/40 rewritten to prove both halves: repeat-save reuse of an already-tagged Contact, and an ordinary same-name dossier Contact surviving completely untouched (with the Job Board save creating a second, distinct, properly-tagged Contact instead).
3. **One remaining canonical-mutation caller still resolved through the flexible SEARCH helpers, plus one caller feeding a Contact mutation via linkedIntelIds.** `GMFactionRelationshipSurfaceController`'s `hide-contact` action still read the target Contact via `findFactionContact(factionId || factionName, contactId || contactName)` before mutating it — the exact SEARCH-helper-feeds-a-mutation pattern this whole phase exists to close, one layer removed (reading the wrong same-named record's data is just as unsafe as mutating by the wrong id). Independent review's requested final grep also found `FactionIntelBridgeService.resolveFaction()`/`resolveContact()` used the same flexible helpers, and `createDraftFromContact()` ultimately mutates the resolved Contact's `linkedIntelIds` via `#linkIntelToContact()` — a real, if easy-to-miss, mutation-target-resolution path. Fixed: two new PUBLIC methods on `FactionRegistryService`, `resolveFactionForMutation(idOrName)` and `resolveFactionContactForMutation(factionIdOrName, contactIdOrName)` — thin public wrappers around the existing PERMISSIVE private resolvers (id wins when it matches; a name is honored only when it names exactly one canonical record; 2+ is refused) — for exactly this "read safely enough to feed a mutation" need. `hide-contact` and `FactionIntelBridgeService.resolveFaction()`/`resolveContact()` now both route through these and throw/report ambiguity rather than guessing. The full final grep independent review asked for (every remaining `findFaction()`/`findFactionContact()` call site, classified READ-ONLY vs MUTATION-TARGET-RESOLUTION) confirmed every other remaining call site is genuinely read-only or already-id-only: `getFactionContacts()`'s and `promoteFactionContactToActor()`'s internal re-fetches by already-resolved real ids; `GMFactionRelationshipSurfaceService.js`'s `registryMissing` display flag; `AlliesSurfaceService.js`'s `alignedWithFactionId`/`alignedAgainstFactionId` (populated only from a real-id select option, never free text); and `GMJobBoardSurfaceService.js`'s `resolveIssuerFaction()`/`resolveIssuerContact()`/`resolveConsequenceFactions()`, which turned out to already implement this exact graduated id-first/unique-name/ambiguous-refused policy independently (a presentation-layer precedent for the same pattern, not a gap). New test 41 covers both fixed call sites.

None of these three fixes required touching Job/Location/Intel architecture, adding a new canonical authority, or introducing a generic identity/migration framework — `legacyFactionIdCandidate()`/`legacyContactIdCandidate()` and the two new public resolver methods are small, narrowly-scoped, domain-specific additions reusing the same private resolvers and `mintUniqueId()` seam round 1 already built.

Re-ran full validation after this second correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 36/36 assertion-groups pass (34 prior + 2 net-new: 9b and 41, plus 9/10/19/37/39/40 rewritten in place for the new expected behavior); `gm-faction-ecosystem-view-model`/`gm-job-ecosystem-view-model`/`gm-locations-ecosystem-view-model`/`gm-intel-ecosystem-view-model`/`gm-generation-phase8d3b/8d3c/8d1`/`gm-datapad-no-duplicate-handler-regression`/`gm-campaign-context-parity`/`gm-campaign-context-read-only-contract`/`gm-faction-context-navigation-controller`/`gm-workspace-faction-contact-navigation-controller`/`gm-holonet-phase8b/8c` all unaffected; `node tools/run-rolling-tests.mjs`, `node tools/run-rolling-syntax-check.mjs`, `node tools/check-architecture-boundaries.mjs` (same 37 pre-existing findings), `node tools/validate-partials.mjs`, `node tools/validate-data.js`, and `system.json` parse were all re-run clean.

### Correction pass round 3 (independent re-re-review of round 2)

A third independent re-review confirmed round 2's migration-compatibility fix was correct, and found two more narrow identity leaks — the Job Board's own round-2 fix still inferred Contact sameness from a name match in a case where a real canonical id was already available, and the Intel bridge's write-back, though routed through the new ambiguity-safe resolver at its entry point, still passed already-resolved ids back through the flexible search helper one step later:

1. **The Job Board discarded the canonical `issuerFactionId`/`issuerContactId` it already had, then reconstructed identity from a name.** The contract-submit handler already builds the Job's own `issuer` object from `text('issuerFactionId')`/`text('issuerContactId')` (populated when the GM picks a known issuer from the dropdown) — but the very next block, `if (data.get('clientSave') === 'on') { await this._saveClientAsContact({ factionName: text('clientFaction'), clientName: text('clientName'), ... }) }`, never passed those same ids through, so `_saveClientAsContact()` always fell back to its round-2 tag-gated name lookup even when the exact canonical Contact was already known — display text still deciding canonical sameness, one layer above the registry, in the specific case where it didn't need to at all. **Fixed:** `_saveClientAsContact()` now accepts `factionId`/`contactId` parameters; the caller passes `factionId: text('issuerFactionId')`, `contactId: text('issuerContactId')`. Policy: an explicit `factionId` resolves authoritatively via `resolveFactionForMutation()` (ambiguous → throws, never guesses among duplicate same-named Factions, regardless of how many share that name); an explicit `contactId` resolves via `resolveFactionContactForMutation()` and updates exactly that Contact; only when NEITHER was ever available (a purely free-text client/faction save with no known-issuer selection) does it fall back to the round-2 tag-gated name-match legacy path, now explicitly scoped to that one narrow case rather than the general rule.
2. **`FactionIntelBridgeService#linkIntelToContact()` still resolved its write-back target through `findFactionContact()`.** `resolveFaction()`/`resolveContact()` (round 2's fix) correctly gate the INITIAL resolution, but `createDraftFromContact()` passes the already-resolved `faction.id`/`contact.id` into `#linkIntelToContact()`, which re-resolved them via `FactionRegistryService.findFactionContact(factionId, contactId)` — internally `findFaction()`, a single combined-condition `.find()` (`record.id === query || ... || record.name.toLowerCase() === needle || ...`) that can return the WRONG record when some OTHER Faction's display name happens to equal the target's real id and that other Faction sorts earlier in the registry array. Passing real ids through it was safe in the common case but not structurally so — exactly the "statistically safe, not structurally correct" gap this whole phase exists to close. **Fixed:** `#linkIntelToContact()` now resolves through the same `resolveFactionContactForMutation()` used everywhere else, throwing on ambiguity rather than ever reaching the vulnerable combined-condition search. A regression test (45) constructs the exact hazard — a decoy Faction named literally `"faction-x"`, created before a target Faction whose real id is `"faction-x"` — and proves the Intel link still lands on the correct, id-targeted Contact.

A full final grep of every remaining `findFaction()`/`findFactionContact()` call site (the same audit round 2 ran, re-verified after these two fixes) confirms no further occurrence sits upstream of a canonical mutation: `getFactionContacts()`'s and `promoteFactionContactToActor()`'s/`upsertFaction()`'s internal re-fetches are by already-resolved real ids; `GMFactionRelationshipSurfaceService.js`'s `registryMissing` flag and the three `GMJobBoardSurfaceService.js` issuer-resolution functions are read-only presentation (the latter already independently implementing this same graduated policy); `AlliesSurfaceService.js`'s aligned-Faction fields are populated only from a real-id select option; and `holonet-messenger-service.js`'s Job-consequence resolution remains the one documented, frozen, name-only exception (Job architecture out of scope, no other identity signal exists for that field).

None of these two fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — both reuse the exact `resolveFactionForMutation()`/`resolveFactionContactForMutation()` seam round 2 already built.

Re-ran full validation after this third correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 41/41 assertion-groups pass (36 prior + 5 net-new: 42-46; test 39's replica and test 40's structural guard updated in place to reflect the new explicit-id-first algorithm); all previously-listed adjacent ecosystem/generation/navigation/holonet tests unaffected; `node tools/run-rolling-tests.mjs`, `node tools/run-rolling-syntax-check.mjs`, `node tools/check-architecture-boundaries.mjs` (same 37 pre-existing findings), `node tools/validate-partials.mjs`, `node tools/validate-data.js`, and `system.json` parse all re-run clean.

### Correction pass round 4 (independent re-re-re-review of round 3)

A fourth independent re-review found one remaining leak in the Job Board's own `_saveClientAsContact()` — the last place display text could still, in one specific circumstance, decide canonical Contact sameness, and the last place a known-but-unresolvable canonical id could be silently discarded rather than failing loudly:

1. **An explicit `contactId` that failed to resolve was silently treated as "no id was ever supplied," falling through to create a new Contact instead of failing.** Round 3's fix correctly used an explicit `contactId` authoritatively when it resolved — but if it did NOT resolve (a stale id left over from a deleted Contact, or one that actually belongs to a different Faction than the one just resolved for this save), the code computed `existingContactId = contactResolution.contact?.id || ''` and fell straight through to `upsertFactionContact()` with an empty id, i.e. silently created a brand-new Contact instead of surfacing the failure. A GM re-confirming a known issuer whose record had since been deleted, or a cross-Faction id mismatch from a stale form state, would never see an error — they'd just get an unexpected extra Contact. **Fixed:** resolution failure and ambiguity are now both hard errors — `if (!contactResolution.contact) throw new Error('The selected issuer Contact could not be found on this Faction.')` — there is no fall-through path left. New regression tests 39a (valid `factionId` + stale `contactId` → throws, creates no Contact) and 39b (`factionId` resolving to Faction A + `contactId` actually belonging to Faction B → throws, creates no Contact on A, leaves B's real Contact completely untouched) cover both failure shapes.
2. **The free-text (no explicit `contactId`) path still reused an existing Contact by name, even though it was tag-gated.** Round 2 added a `['job-board', 'reusable-contact']` tag gate specifically so a bare free-text name match couldn't overwrite an ordinary dossier Contact — but the tag gate never addressed the deeper issue: it still let a NAME (now name-plus-tags, but still display text) decide that two separately-typed free-text saves referred to the SAME canonical Contact. That is exactly the class of leak this whole phase exists to close, one narrow case removed. **Fixed:** the free-text path no longer performs any Contact lookup at all. The contract is now unconditional: `explicit contactId → exact Contact under the resolved Faction, or FAIL` / `no contactId → ALWAYS create a new Contact`. The only way to update an existing Contact is to already hold its real canonical id (the GM selecting a known/reusable Contact in the wizard, which populates `issuerContactId` and lands in the id-first branch) — typing a bare name, tagged reuse-candidates included, now always means "this is a new Contact." Test 39 was rewritten (its old "two free-text saves of the same name reuse one Contact" premise is retired) to instead prove: two free-text saves of the same name produce two distinct Contacts; selecting the first one later via its real `contactId` updates exactly that Contact and leaves the second untouched; an ordinary same-name dossier Contact remains untouched. Test 43's still-valid "two same-name reusable Contacts coexist, a further free-text save never merges into either" assertions were kept, with its comment/wording corrected — round 2/3's premise that this only held because the free-text match was *ambiguous* is superseded; the rule no longer depends on ambiguity detection at all, it applies unconditionally.

A final negative-check grep of `_saveClientAsContact()`'s isolated method body (also promoted to a permanent structural regression test, 40) confirms no code path anywhere in the method assigns an existing Contact's id from `entry.name`, `contactName`, name equality, or name-plus-tags — the ONLY assignment to `existingContactId` in the entire method is `contactResolution.contact.id`, reached exclusively through the explicit-`contactId`-resolves-or-throws branch.

**Correction to prior rounds' wording:** round 2's and round 3's correction-pass write-ups above (marked at the time as the accepted, shipped behavior) described a tag-gated name-match reuse path as intended Job Board behavior for the no-explicit-id case. That description is now **obsolete** — round 4 removed that path entirely. Those sections are left as an unedited historical record of what shipped in each round; this note is the authoritative correction. The Final verdict below reflects only the current, post-round-4 state.

None of these fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — both reuse the exact `resolveFactionForMutation()`/`resolveFactionContactForMutation()` seam rounds 2-3 already built; the change is a deletion (the tag-gated name-reuse branch) and a fall-through-to-throw tightening, not new surface area.

Re-ran full validation after this fourth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 43/43 assertion-groups pass (41 prior + 2 net-new: 39a, 39b; tests 39, 40, and 43 rewritten in place to reflect the unconditional always-create-on-no-id rule); all previously-listed adjacent ecosystem/generation/navigation/holonet tests unaffected; `node tools/run-rolling-tests.mjs`, `node tools/run-rolling-syntax-check.mjs`, `node tools/check-architecture-boundaries.mjs` (same 37 pre-existing findings), `node tools/validate-partials.mjs`, `node tools/validate-data.js`, and `system.json` parse all re-run clean.

### Correction pass round 5 (independent re-review of round 4's head commit)

A fifth independent review, against round 4's exact head commit (`5cc5f7c08`), confirmed the free-text Contact identity fix was correct and exact-head CI was green — but found one remaining structural gap: the two "explicit id" branches round 4 added looked id-safe on the surface, but the PUBLIC resolvers they called, `resolveFactionForMutation()`/`resolveFactionContactForMutation()`, are — by design, per their own doc comment — PERMISSIVE id-or-unique-name compatibility resolvers, not canonical-id validators. Concretely: if a Faction named `"stale-faction-id"` exists, and the Job Board is asked to resolve the explicit id `"stale-faction-id"` (which matches no real Faction id), `resolveFactionForMutation()` would try the string as an id (no match), then fall back to trying it as a unique display name (a match) — silently "rescuing" a failed canonical-id lookup with a same-named record's identity. The same hazard existed one layer down for `resolveFactionContactForMutation()` (a stale `contactId` colliding with some other Contact's display name), and in `FactionIntelBridgeService#linkIntelToContact()`, which passes already-canonical `faction.id`/`contact.id` through the same permissive resolver for its write-back.

**Fixed, narrowly, by adding an EXACT-ID-ONLY resolution seam alongside the existing PERMISSIVE one, rather than changing the permissive resolvers' contract** (which other, legitimately permissive callers — `hide-contact`, `FactionIntelBridgeService.resolveFaction()`/`resolveContact()`'s own entry points, both of which genuinely only ever carry one combined id-or-name string — still correctly depend on):

- `FactionRegistryService.resolveFactionByIdForMutation(factionId)` — exact `record.id === factionId` match or `null`. No name fallback, no slug fallback, no case-normalized fallback, ever.
- `FactionRegistryService.resolveFactionContactByIdsForMutation(factionId, contactId)` — exact Faction id required, then exact Contact id on that Faction required, or `{ contact: null }`. No name fallback at either level.

`GMJobBoardSurfaceController._saveClientAsContact()`'s two explicit-id branches now call these instead of the permissive resolvers: an explicit `factionId`/`contactId` that resolves to nothing throws, full stop — it is never "rescued" by a coincidentally-matching display name anywhere in the registry. `FactionIntelBridgeService#linkIntelToContact()`'s write-back target resolution was changed the same way; its own public `resolveFaction()`/`resolveContact()` entry points remain permissive, since they explicitly accept id-OR-name input from external callers — only the internal write-back, which already holds canonical ids, was tightened.

Six new adversarial regression tests construct the exact collision this gap depended on and prove it closed: 39c/39d (a Faction/Contact respectively whose DISPLAY NAME literally equals the queried-but-nonexistent id — must throw, never resolve to the name-colliding record, direct resolver-level assertions plus the `_saveClientAsContact()` integration path); 39e (point C — Faction A holds a Contact NAMED exactly Faction B's real Contact id; querying Faction A with that string as the `contactId` must throw, touching neither the name-colliding decoy on A nor B's real Contact); 45b (the same hazard against the Intel write-back, with a decoy Contact on the same Faction). Test 40's and 46's structural guards were extended to assert the explicit-id branches use the new exact-id resolvers and never the permissive ones.

None of this touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic identity framework — two small, narrowly-scoped methods reusing the exact same registry/`normalizeContact()`/`getRegistry()` machinery every other resolver here already uses, plus a caller-side swap at the two sites that needed exact-id semantics.

Re-ran full validation after this fifth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 47/47 assertion-groups pass (43 prior + 4 net-new: 39c, 39d, 39e, 45b; tests 40 and 46 extended in place); all previously-listed adjacent ecosystem/generation/navigation/holonet tests unaffected; `node tools/run-rolling-tests.mjs`, `node tools/run-rolling-syntax-check.mjs`, `node tools/check-architecture-boundaries.mjs` (same 37 pre-existing findings), `node tools/validate-partials.mjs`, `node tools/validate-data.js`, and `system.json` parse all re-run clean.

**Resolver contract, made explicit (documented at the resolver definitions in `faction-registry-service.js` and here for reference):**

- canonical id already known (a real, already-resolved id in hand) → **exact-id-only**: `resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`. A non-matching id fails; a same-named other record can never rescue it.
- legacy/free-text combined input (a caller that genuinely only ever has ONE string, which may be an id or a display name) → **id-or-unique-name compatibility seam**: `resolveFactionForMutation()` / `resolveFactionContactForMutation()` (and their private counterparts). These remain intentionally permissive for their existing callers; they are not, and were never meant to be, canonical-id validators.
- display text can never rescue a failed EXPLICIT canonical-id lookup, in either resolver family's correct usage — the distinction is which family a given caller's input shape calls for, not a weakening of either one.

### Correction pass round 6 (independent re-re-re-re-re-review)

A sixth independent review, against round 5's head commit, confirmed the exact-id resolver seam was correct — and found one remaining issue at a deeper layer than any prior round: the canonical AUTHORITY itself, `upsertFaction()`/`upsertFactionContact()`, not merely another caller. Every prior round closed a leak where some CALLER could still, in one path, let display text decide identity, or let a resolved-but-wrong id substitute for a failed lookup. Round 6's finding was structurally different: the shared creation/update boundary both methods sit behind still computed `existing?.id || requestedId || randomId()` — an explicit id that matched nothing simply became the id of a brand-new record. A caller passing a stale, deleted, or outright fabricated id (`upsertFaction({ id: 'stale-or-fabricated-id', name: 'Some Faction' })`) would silently get a NEW canonical Faction minted under that exact id, rather than the update it presumably intended, or a failure. This is the one hole no amount of caller-side hardening (rounds 1-5) could close, because it lives in the authority both permissive and exact-id resolvers ultimately call through to create.

**Audit before changing the contract (full results in the commit and PR body):** grepped every `upsertFaction(`/`upsertFactionContact(` call site across the codebase and classified each as NORMAL CREATE (no id supplied), NORMAL UPDATE (id supplied, backed by an already-real, already-resolved record), or a potential LEGACY MATERIALIZATION dependency. Four internal call sites inside `FactionRegistryService` itself — `addActorRelationship()`, `updateActorRelationship()`, `applyScoreDelta()`, `approveSuggestedFaction()` — each had a fallback of the shape `resolution.faction || await this.upsertFaction({ id: possiblyStaleId, ... })`: if an explicit id was supplied and failed to resolve, the id was passed straight through, materializing a phantom Faction under it. Every OTHER call site (in `AlliesSurfaceService`, `GMFactionRelationshipSurfaceController`, `GMLocationsSurfaceController`, `GMWorkspaceSurfaceController`, `GMJobBoardSurfaceController`, `FactionIntelBridgeService`) was independently verified to already pass either no id (create) or an id that was itself just resolved from a real, existing record (update) — none of them exercise the vulnerable path today. Tracing every production caller of `suggestFaction()` (the only feeder of `approveSuggestedFaction()`'s `merged.factionId`) confirmed it never sets a `factionId` on a suggestion in the first place, so **no legitimate LEGACY MATERIALIZATION caller exists anywhere in production** — per the audit's own decision rule, the create-with-caller-chosen-id behavior was removed entirely rather than preserved behind a compatibility seam.

**Fixed:** `upsertFaction()`/`upsertFactionContact()` now enforce, at the authority's own boundary:
- id supplied, matches an existing record → update exactly that record
- id supplied, matches **nothing** → **throw** (`No Faction/Contact exists with id "…"`), never create
- no id supplied → create; the authority mints the id itself (`randomId()`), never a caller-chosen or name-derived one

The four internal fallback call sites were tightened to match: `addActorRelationship()`/`updateActorRelationship()` now throw when an explicit `factionId` fails to resolve (only a purely name-only call, with no id ever supplied, may create); `applyScoreDelta()` fails SOFT (returns `null`, logs) on the same condition, consistent with its existing batch-safe contract, so one actor's stale reference can't abort an unrelated actor's update in the same job-consequence batch; `approveSuggestedFaction()` throws, since the audit found no production suggestion ever carries a non-empty `factionId` in the first place.

**Test-fixture correction (point 7 of the review):** test 45's decoy-collision fixture previously called `upsertFaction({ id: 'faction-x', name: 'Faction X' })` to create a Faction under a deliberately-chosen id that didn't exist yet — itself now the exact pattern round 6 closes. Rewritten to seed the raw registry directly (the same pattern test 8 already used for a pre-existing legacy id), preserving the decoy-created-first array ordering the test depends on, without weakening the production authority's contract to accommodate the test.

Six new regression tests (47-52, matching the review's lettered points A-E, tests 47/48 direct-authority, 49/50/51/52 caller-level) prove: `upsertFaction()`/`upsertFactionContact()` throw and leave the registry/Contact-list unchanged when given an unknown id; `addActorRelationship()` refuses to materialize a Faction from an unresolvable explicit `factionId` while its name-only creation path still works and mints its own id; `updateActorRelationship()` refuses to resurrect a deleted Faction from a relationship's now-stale `factionId`; `applyScoreDelta()` fails soft with no Faction or relationship created; `approveSuggestedFaction()` refuses to create a Faction from a stale suggestion `factionId`. Test 53 (point F) proves the frozen name-only Job-consequence path still works unchanged, and that any Faction it creates gets an authority-minted id, never name-derived or caller-provided text. Test 54 is a structural regression guard confirming none of the four internal fallback call sites still pass a possibly-unresolved id through to `upsertFaction()`.

None of this touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — the fix tightens an existing boundary (two `if` guards at the authority, four call-site adjustments removing an id argument or adding a fail-check) rather than adding surface area.

Re-ran full validation after this sixth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 55/55 assertion-groups pass (47 prior + 8 net-new: 47-54; test 45's fixture corrected in place); all previously-listed adjacent ecosystem/generation/navigation/holonet tests unaffected, plus a fresh check of every `AlliesSurfaceService`-adjacent test (`follower-slot-occupancy-alignment`, `gm-campaign-context-*`, `gm-existing-npc-allies-assignment`, `gm-manual-follower-slot`, `gm-npc-assignment-modal`, and the rest) given this round touched Actor-relationship mutators directly; `node tools/run-rolling-tests.mjs` 195/195 pass; `node tools/run-rolling-syntax-check.mjs` 2459/2459 clean; `node tools/check-architecture-boundaries.mjs` unchanged at 37 pre-existing findings; `node tools/validate-partials.mjs` and `node tools/validate-data.js` clean; `system.json` parses.

### Correction pass round 7 (independent re-review of round 6's exact head commit)

A seventh independent review confirmed round 6's authority-level create/update contract was correct, and found three further issues — two structural, one a caller-semantics cleanup:

1. **Ordinary canonical id minting was not structurally collision-safe.** `migrateLegacyIdentities()` already reserves every existing id and mints through `mintUniqueId(reserved)` specifically because canonical-id uniqueness must be structural, not merely "`randomID()` is astronomically unlikely to repeat." Ordinary creation (`upsertFaction()`/`upsertFactionContact()`) still minted via plain `randomId()`. An unlucky (or, in a test, stubbed) `randomID()` draw could produce two canonical records sharing one id between creation and the next migration sweep — worse for Contacts, since round 1 already established Contact ids as one pool across every Faction, not merely unique within the parent Faction. **Fixed:** both now mint through `mintUniqueId()`, reserving every existing Faction id (for `upsertFaction()`) or every Contact id across the ENTIRE registry (for `upsertFactionContact()`, matching the registry-global policy). Two new tests (55, 56) stub `randomID()` to force a first-draw collision on ordinary Faction and Contact creation respectively and prove the retry produces a structurally unique id.
2. **`findFaction()`/`findFactionContact()` let an earlier record's DISPLAY NAME shadow a later record's real canonical id.** Both were a single combined `.find()` predicate (`record.id === query || ... || record.name.toLowerCase() === needle || ...`) evaluated per record in array order — so a decoy record whose NAME equals a later record's real id, sorted earlier in the registry array, would win. This was not theoretical: test 45 (round 3) had to explicitly route around `findFaction()` for its own fixture setup, and its comment said so — the test suite itself was documenting the still-open gap. **Fixed:** both now resolve in two passes over the full registry — id match (exact, then legacy case-insensitive) always evaluated first across every record; name/slug is only ever tried once no id matched anything. This preserves all existing flexible-search behavior; it only reorders precedence so a real id can never be shadowed. New tests 57/58 construct the exact hazard and prove the fix; test 45's own comment and assertions were updated to confirm `findFaction()` itself is now also safe for the adversarial shape it used to have to avoid.
3. **`FactionJobBridgeService`'s own Contact lookup had the identical precedence bug, independently** (`buildDraftFromContact()`/`issuerFilterFromContact()` resolve against a `faction.contacts` array directly, not through `findFactionContact()`, so fixing the registry's own helpers didn't reach it) — a real, not merely presentational, path: the resolved Contact's id gets copied into a persisted Job draft's `issuer.contactId`. **Fixed:** a new shared `resolveContactByIdThenName()` helper applies the identical id-priority precedence. New tests 59-61 prove `buildDraftFromFaction()`, `buildDraftFromContact()`, and `LocationJobBridgeService`'s `controllingFactionId` resolution (which already went through the now-fixed `findFaction()`) all resolve the exact id-bearing record even under a name-collision decoy. Test 62 is a structural regression guard.
4. **A handful of `GMFactionRelationshipSurfaceController` dossier actions (`hide-contact`, `promote-contact`, `delete-contact`, `make-job-*`, `create-intel-*`, `reveal-*`) had genuinely separate `factionId`/`factionName`/`contactId`/`contactName` fields from the button dataset, but collapsed them into one `factionId || factionName` / `contactId || contactName` string before resolving.** That collapse loses the distinction between "this IS a real id reference" and "this is free text": if a real id field were ever stale (a deleted/renamed record, a race between render and click), the collapsed string would fall through to the flexible resolvers' name-match pass — the same class of hazard rounds 5-6 closed elsewhere, reintroduced one caller-layer up. Audited every one of these actions: the id fields are always rendered directly from the currently-displayed, already-real Faction/Contact row, so a real id should always be present and always resolve — this is a defense-in-depth closure, not a currently-reachable bug in normal UI flow. **Fixed narrowly:** two new local helpers, `resolveIssuerFaction(factionId, factionName)` and `resolveIssuerContact(faction, contactId, contactName)` — when the id field is non-empty, resolve EXACT-ID-ONLY (via the round-5 exact-id resolvers) and fail rather than ever falling back to name; only when the id field is genuinely empty is name-based compatibility resolution used. All six action groups now resolve through these before calling into `FactionJobBridgeService`/`FactionIntelBridgeService`/`FactionRegistryService`'s own mutators, passing the verified real object (or id) through rather than the collapsed string. New tests 63/64 (replica, proven against the real `FactionRegistryService`) construct a stale id whose text equals another record's real display name and prove it throws rather than resolving to that other record; test 65 is a structural regression guard confirming no collapsed `factionId||factionName`/`contactId||contactName` call argument remains anywhere in the controller.

None of these four fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — `mintUniqueId()` is the exact helper migration already uses; the two-pass id-priority reorder is a precedence change to two existing search helpers, not new logic; `resolveContactByIdThenName()`/`resolveIssuerFaction()`/`resolveIssuerContact()` are small, narrowly-scoped local helpers reusing the registry's own round-5 exact-id resolvers.

Re-ran full validation after this seventh correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 66/66 assertion-groups pass (54 prior + 12 net-new: 55-65, an even dozen given 55 and 56 are a matched pair; test 45's comment and assertions updated in place); all previously-listed adjacent ecosystem/generation/navigation/holonet/Allies-surface tests unaffected; `node tools/run-rolling-tests.mjs` 195/195 pass; `node tools/run-rolling-syntax-check.mjs` 2459/2459 clean; `node tools/check-architecture-boundaries.mjs` unchanged at 37 pre-existing findings; `node tools/validate-partials.mjs` and `node tools/validate-data.js` clean; `system.json` parses.

### Correction pass round 8 (independent re-review of round 7's exact head commit)

An eighth independent review confirmed round 7's fixes were themselves correct, and found three further issues — two of which were the same structured-id-collapse bug round 7 had just removed, resurfacing on the Job side:

1. **`findFaction()`/`findFactionContact()`'s own id pass was itself still one combined exact-or-case-insensitive predicate.** Round 7 fixed display-NAME shadowing by moving id resolution ahead of name resolution — but within that id pass, `record.id === cleanQuery || record.id.toLowerCase() === needle` still evaluated both conditions per record in array order, so an EARLIER Faction matching only case-insensitively (e.g. an id of `"FACTION-X"`) could still shadow a LATER Faction's real exact-case id (`"faction-x"`). **Fixed:** genuinely three passes over the full registry now — exact id always wins outright; legacy case-insensitive id compatibility only when it names exactly ONE Faction (never guessed among 2+ differently-cased ids, matching the ambiguity-refusal convention used everywhere else in this file); name/slug only once no id match won at all. Same fix applied to `findFactionContact()`. New tests 67/68 construct the exact case-only-collision hazard and prove it closed, while confirming unique case-insensitive compatibility still works.
2. **`GMJobBoardSurfaceService`'s `filterDraft` construction, and `GMJobBoardSurfaceController`'s follow-up-contract handler, both independently collapsed the Job Board's structured `issuerFilter`** (separate `factionId`/`factionName`/`contactId`/`contactName` fields) **into one ambiguous string** (`issuerFilter.factionId || issuerFilter.factionName`, `filter.contactId || filter.contactName`) **before calling into `FactionJobBridgeService`** — the exact bug round 7 had just removed from the Faction dossier controller (`GMFactionRelationshipSurfaceController`'s `resolveIssuerFaction()`/`resolveIssuerContact()`), reintroduced independently on the Job-side consumers of that same `issuerFilter` shape. Round 7's audit of "every caller that collapses separate id/name fields" covered the Faction dossier controller but not these two Job Board call sites. **Fixed:** a new `FactionJobBridgeService.buildDraftFromIssuerFilter(filter)` is the single seam for building a Job draft from this structured shape — a populated id field resolves EXACT-ID-ONLY and fails (returns `null`) rather than ever falling back to the sibling name field; only a genuinely empty id field permits the unique-name compatibility path. Both call sites now route through it instead of duplicating (and re-breaking) the collapse-then-resolve logic independently. New test 69 proves the seam directly (a stale `factionId` whose text equals another Faction's real name must never build a draft for that other Faction) plus a structural guard on `GMJobBoardSurfaceService`'s source; test 70 is a structural guard on `GMJobBoardSurfaceController`'s follow-up-contract handler (exercising the real DOM click handler is out of scope for this identity-focused test file — the underlying seam it now calls is already proven functionally in test 69).
3. **`FactionJobBridgeService`'s round-7 id-priority fixes never made the NAME fallback ambiguity-safe.** `resolveContactByIdThenName()`/`resolveFactionIdOrUniqueName()`'s predecessor logic correctly made a real id outrank a name match, but the name-match branch itself was still a plain `.find()` — two Factions (or two Contacts on one Faction) sharing a display name would silently resolve to whichever sorts first. Unsafe specifically because this bridge copies the resolved record's CANONICAL id into a persisted Job draft's `issuer.factionId`/`issuer.contactId` — a silent guess stamps the wrong canonical identity into that draft. **Fixed:** `resolveContactByIdThenName()`'s name-match branch now requires exactly one match (2+ → `null`, refusing to guess); `buildDraftFromFaction()`/`buildDraftFromContact()`/`issuerFilterFromFaction()`/`issuerFilterFromContact()`'s string-resolution path was changed from the flexible, always-guessing `FactionRegistryService.findFaction()` SEARCH helper to a new local `resolveFactionIdOrUniqueName()`, which reuses `FactionRegistryService.resolveFactionForMutation()` — already the correct ambiguity-safe id-or-unique-name resolver (its name is mutation-oriented, but its actual contract, refusing to guess among duplicate-name Factions, is exactly what this read-only bridge needed too; no new framework introduced). New tests 71/72 prove two same-name Factions/Contacts are refused rather than guessed (test 72 also confirms `buildDraftFromContact()`'s pre-existing "Contact not found → fall back to a Faction-only draft" behavior correctly extends to the ambiguous case too — a Faction-only draft carries no `contactId` field at all, so no wrong Contact identity is ever stamped even in that softer failure mode); unique-name and exact-id lookups continue to work, including with duplicate names present elsewhere on the same Faction.

A full re-audit of every remaining `issuerFilter.factionId || issuerFilter.factionName` / `contactId || contactName`-shaped pattern (and every `buildDraftFromFaction()`/`buildDraftFromContact()` call site) found nothing else in this shape: the handful of remaining `||`-chained `factionId`/`factionName` occurrences elsewhere in the codebase are either genuine READ-ONLY display/merge code (never feeding a mutation or a draft's stamped identity — `GMFactionRelationshipSurfaceService`'s `registryMissing` flag, `AlliesSurfaceService`'s legacy-data view-model merge) or read-time NORMALIZATION of a raw persisted record's own possibly-missing field (`_relationshipRow()`, `_normalizeActorRelationship()` — the same established "recover a legacy record's effective identity" pattern round 2 already covers, not a live two-field collapse).

**Correction to prior rounds' wording (round 7):** the "Implemented policy" section above (round 1) still describes new Faction/Contact id generation as plain `randomId()`. Round 7 upgraded ordinary creation to the structurally collision-safe `mintUniqueId()`; see that section's own correction note and round 7's write-up above.

None of these three fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — `buildDraftFromIssuerFilter()` is a small, narrowly-scoped seam reusing the exact exact-id resolvers round 5 already built; the three-pass search reorder and the ambiguity-safe name fallback are precedence/refusal changes to existing logic, not new surface area.

Re-ran full validation after this eighth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 72/72 assertion-groups pass (66 prior + 6 net-new: 67-72); all previously-listed adjacent ecosystem/generation/navigation/holonet/Allies-surface tests unaffected, plus every Job/Faction ecosystem, navigation, and 8D-3 generation test given this round touched `GMJobBoardSurfaceService`/`GMJobBoardSurfaceController`/`FactionJobBridgeService` directly; `node tools/run-rolling-tests.mjs` 195/195 pass; `node tools/run-rolling-syntax-check.mjs` 2459/2459 clean; `node tools/check-architecture-boundaries.mjs` unchanged at 37 pre-existing findings; `node tools/validate-partials.mjs` and `node tools/validate-data.js` clean; `system.json` parses.

### Correction pass round 9 (independent re-review of round 8's exact head commit)

A ninth independent review confirmed round 8's Job Board `issuerFilter` fixes were themselves correct, and found four further narrow closure issues — all instances of "fail closed on ambiguous/stale canonical identity" not yet applied consistently at the last few remaining edges:

1. **`LocationJobBridgeService` still resolved a Location's canonical `controllingFactionId` through the flexible, name-capable `findFaction()` SEARCH helper**, even though that field is unambiguously canonical (never free text). A stale/deleted `controllingFactionId` that happened to equal some OTHER (unrelated) Faction's display name would be silently "rescued" by that Faction — producing an internally inconsistent Job draft whose `issuer.factionId` (the stale id, copied through verbatim) and `issuer.factionName`/`client.factionName` (the rescued, unrelated Faction's name) described two different entities. The existing round-7 regression test only proved a *valid* `controllingFactionId` outranks a name collision; it never exercised a *stale* one being rescued by unrelated display text. **Fixed:** `findFactionName()` now resolves via `FactionRegistryService.resolveFactionByIdForMutation()` (EXACT-ID-ONLY) instead of `findFaction()`. An unresolved id now correctly yields no name (`''`) — still paired with its own (stale) id verbatim in `issuer.factionId`, which is internally consistent (both say "unknown"), rather than a name borrowed from an unrelated record. New test 73 constructs the exact hazard and proves it closed, while confirming a genuinely resolvable `controllingFactionId` still works.
2. **`buildDraftFromContact()`/`issuerFilterFromContact()` silently downgraded a NON-EMPTY but unresolved-or-ambiguous Contact selector into a Faction-only result**, rather than failing outright. Round 8's own new test 72 explicitly encoded this downgrade as the desired behavior — safe in the narrow sense that no *wrong* Contact id was ever stamped into the draft, but it silently changed the caller's requested entity ("build from Contact X") into something else ("build from the Faction") without any signal that the substitution happened. **Fixed:** both methods now distinguish "no Contact selector was ever supplied" (Faction-level fallback remains correct and intentional) from "a Contact selector was supplied but failed to resolve, not-found or ambiguous alike" (now returns `null` outright, never substituting the Faction). Test 72 was rewritten to prove the `null` contract directly (superseding its round-8 "degrades to Faction-only" assertions); new test 74 covers the "not found" shape specifically (as distinct from test 72's "ambiguous" shape) for both methods, and confirms the genuinely-no-selector-supplied case still correctly falls back to Faction-level.
3. **`findFaction()`/`findFactionContact()`'s case-insensitive-id pass still fell through to a display-name match on 2+ ambiguous candidates**, rather than refusing outright the way every other ambiguity in this file does. Round 8 correctly split exact-id, case-insensitive-id, and name into three passes, but only handled the "exactly one case-insensitive match" case explicitly — both "zero matches" (correctly falls through to name) and "two or more matches" (should refuse) took the same fall-through path. That let a completely unrelated *third* record's display name decide among two id-shaped candidates it has nothing to do with. **Fixed:** 2+ case-insensitive id matches now return `null` immediately, never reaching the name/slug pass. New tests 75/76 construct the exact three-record hazard (two differently-cased ids plus an unrelated record whose *name* equals the ambiguous query) for both Factions and Contacts and prove the refusal.
4. **`FactionJobBridgeService`'s Contact name-fallback compared names case-SENSITIVELY**, unlike the registry's own name-equality rule (`resolveFactionContactForMutation()` already treats "Mira" and "mira" as the same ambiguous duplicate, case-insensitively). A free-text lookup of `"Mira"` when both `"Mira"` and `"mira"` exist should refuse as ambiguous, not silently treat them as two unrelated names and arbitrarily resolve the exact-case match. **Fixed:** rather than maintain a second, subtly-different local name-matching implementation (exactly the kind of duplicated-resolver drift this whole phase has been closing), `resolveContactByIdThenName()` was rewritten to delegate entirely to `FactionRegistryService.resolveFactionContactForMutation()` — the registry's own ambiguity-safe, case-insensitive id-or-unique-name resolver, already reused for the Faction side via `resolveFactionIdOrUniqueName()`. One resolver, one semantics, reused everywhere this bridge needs "id-or-unique-name, never guess." New test 77 proves `"Mira"`/`"mira"` are now correctly refused as ambiguous duplicates.

None of these four fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — `resolveContactByIdThenName()`'s rewrite is a deletion of duplicated logic in favor of reusing an existing resolver; the other three are refusal/precedence tightenings of existing logic, not new surface area.

Re-ran full validation after this ninth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 77/77 assertion-groups pass (72 prior + 5 net-new: 73-77; test 72 rewritten in place to prove the new `null` contract instead of the now-superseded "degrades to Faction-only" behavior; test 62's structural guard updated for `resolveContactByIdThenName()`'s new `(faction, contactOrId)` signature); all previously-listed adjacent ecosystem/generation/navigation/holonet/Allies-surface tests unaffected, plus every Location ecosystem, Atlas lead, and identity/hierarchy-integrity test given this round touched `LocationJobBridgeService` directly; `node tools/run-rolling-tests.mjs` 195/195 pass; `node tools/run-rolling-syntax-check.mjs` 2459/2459 clean; `node tools/check-architecture-boundaries.mjs` unchanged at 37 pre-existing findings; `node tools/validate-partials.mjs` and `node tools/validate-data.js` clean; `system.json` parses.

### Correction pass round 10 (independent re-review of round 9's exact head commit)

A tenth independent review confirmed round 9's fixes were themselves correct, found three further narrow closure issues, and — separately — the reviewer sent an explicit addendum on release/compatibility posture that reshaped how the third issue was resolved. Both are recorded here.

**Addendum: this system has never shipped, so compatibility burden reverses.** Before this round's fixes were finished, the reviewer clarified that no PRE-8D-4 Faction/Contact registry contract has ever reached a released, persisted user world — there is no real backward-compatibility obligation to any intermediate development revision's behavior. The reviewer's explicit priority order for this and future compatibility questions: (1) clean canonical identity semantics, (2) minimum future technical debt, (3) the smallest number of resolver contracts necessary, (4) compatibility ONLY where a REAL currently-persisted repository/dev datum or an actually-existing production caller requires it. The burden of proof for keeping any compatibility/legacy-preservation seam is now on demonstrating a concrete current caller or persisted-data dependency that needs it — not on demonstrating that removing it is safe. This is a narrow, compatibility-question-scoped directive, not a license for a broad refactor; it governs finding 3 below and is recorded here for future rounds to apply to any similar question.

1. **`LocationJobBridgeService.buildDraftFromLocation()` still copied the RAW, possibly-unresolved `controllingFactionId` verbatim into `issuer.factionId`**, even when round 9's own exact-id-only resolution failed to resolve it. Round 9 fixed the NAME half of this hazard (`issuer.factionName`/`client.factionName` no longer borrow an unrelated Faction's name), but the id itself was still copied through unconditionally — so a broken/stale reference still masqueraded as a resolved canonical Job issuer id. `issuer.factionId` means "this is a resolved canonical Faction reference"; a Location's broken historical reference is useful diagnostic provenance, but must never be presented as one. **Fixed:** a new `resolveControllingFaction(factionId)` helper resolves the reference exactly once and derives BOTH `issuer.factionId` and `issuer.factionName` from that single resolution, so they can never disagree about whether it resolved. On failure both become `''`; the raw broken id survives only as non-canonical diagnostic provenance on `metadata.unresolvedControllingFactionId`, never as `issuer.factionId`. Test 73 was rewritten in place to prove the new contract (superseding round 9's "the raw id is still copied through verbatim" assertion, which described exactly the bug this round closes).
2. **Structured, semantically-explicit `*Name` fields (paired with a sibling `*Id` field that's empty) were still fed through ID-OR-NAME resolvers** — the mirror image of the id-shadowed-by-name bug rounds 5-9 closed. `FactionJobBridgeService.buildDraftFromIssuerFilter()`'s no-id branches called `resolveFactionIdOrUniqueName(factionName)`/`resolveContactByIdThenName(faction, contactName)`, which try the string as an id FIRST — but `filter.factionName`/`filter.contactName` are genuinely separate, structured fields, never a combined id-or-name string, so once the id field is empty the name field is known BY CONSTRUCTION to be a display name, never an id. A Faction/Contact whose real canonical id happened to equal the intended record's display name (e.g. Faction A with id `"Black Sun"`; Faction B with a generated id, actually named `"Black Sun"`) could steal the resolution away from the record that was actually named that. The identical bug existed in `GMFactionRelationshipSurfaceController`'s round-7 `resolveIssuerFaction()`/`resolveIssuerContact()` helpers, which fell back to `FactionRegistryService.resolveFactionForMutation(factionName)`/`resolveFactionContactForMutation(faction.id, contactName)` (the PERMISSIVE id-or-unique-name resolvers) on the identical structured field. **Fixed:** two new public NAME-ONLY resolvers on `FactionRegistryService` — `resolveFactionByUniqueName(name)` and `resolveFactionContactByUniqueName(factionId, name)` — that never try the query as an id at all (case-insensitive display-name equality, refusing to guess among 2+ matches, same ambiguity-refusal convention as every other resolver in this file). `buildDraftFromIssuerFilter()` and `resolveIssuerFaction()`/`resolveIssuerContact()` now route their structured-name-field fallback through these instead. New tests 78-81 construct the adversarial "real id equals another record's real name" scenario for both call sites (Faction and Contact) and prove the name-only resolvers are immune to it; test 65's structural guard was extended to confirm the controller source no longer calls the permissive resolvers on a structured name field.
3. **The permissive id-or-name resolvers (`_resolveFactionByIdOrUniqueName`/`_resolveFactionContactByIdOrUniqueName`) never implemented the "unique case-insensitive legacy id" stage rounds 8-9 added to `findFaction()`/`findFactionContact()`** — a documented-contract divergence between the SEARCH helpers and the mutation-target resolvers. Per the addendum above, this was resolved by **removing** the case-insensitive-id stage entirely, rather than adding it to the permissive resolvers: a source audit confirmed the permissive resolvers never had this stage to begin with, and no real caller or persisted datum anywhere in the codebase depends on two differently-cased id strings resolving as the same canonical Faction/Contact — canonical ids are always machine-generated (`mintUniqueId()`/`randomID()`), so there was never a real reason for `"ABC123"` and `"abc123"` to be the same identity. `findFaction()`/`findFactionContact()` are now simply exact-id, then case-insensitive name/slug — one fewer resolution stage, one fewer place the file's various resolvers could disagree. Tests 67/68 (which proved exact-case-id-outranks-earlier-case-insensitive-match) and 75/76 (which proved case-insensitive-id ambiguity refused rather than falling through to an unrelated record's name) were rewritten in place: they now prove canonical ids are simple exact/case-sensitive matches throughout, with a structural regression guard confirming no `idAmbiguous`-style case-insensitive-id machinery remains in either method.

**One further audit performed while touching this area (not a reviewer finding, but directly requested alongside finding 3): `deleteFactionContact(factionId, contactId)`/`promoteFactionContactToActor(factionId, contactId)`** are named as if both parameters were canonical ids, but still resolved permissively (id-or-unique-name) internally. A caller audit (every production call site of both methods, via `promoteFactionContactToActor(` / `deleteFactionContact(` grep) found exactly two real callers of each: `GMFactionRelationshipSurfaceController`'s promote-contact/delete-contact dossier actions (which already resolve exact-id-first via `resolveIssuerFaction()`/`resolveIssuerContact()` before calling in, or thread a real dataset-sourced `contactId` through directly) and `GMContactActorizerService`'s drag-to-actorize path (`factionId`/`contactId` sourced directly from a drag payload built from an already-rendered real Contact row). No real caller anywhere supplies a bare display name to either parameter. Per the addendum's reversed burden of proof, with no real caller or data requiring the permissive fallback, both methods are **tightened to exact-id-only**, matching their own parameter names. Tightening `deleteFactionContact()` also surfaced and fixed a latent, unrelated honesty bug: it previously returned `true` whenever the Faction resolved, even when no Contact actually matched `contactId` and nothing was removed — it now returns `false` unless a real Contact was found and removed. Test 35 (round-1-era) was updated in place: its assertions that these two methods refuse ambiguous same-named Factions/Contacts with a "Multiple Factions/Contacts named..." error no longer apply (a display name is no longer treated as an ambiguous id-shaped query at all — it simply resolves to nothing, same as any other unresolved id) and now assert the exact-id-only failure/no-op contract instead. New test 82 proves the tightened contract directly, including the return-value fix.

None of these fixes touched Job/Location/Intel architecture, added a new canonical authority, or introduced a generic framework — `resolveControllingFaction()` and the two new NAME-ONLY resolvers are small, narrowly-scoped helpers built from the same exact-id/exact-name primitives every prior round already established; removing the case-insensitive-id stage and tightening two mutators to their own documented parameter contract are both simplifications, not new surface area.

Re-ran full validation after this tenth correction pass: `tests/pre8d4-faction-contact-identity-hardening.test.mjs` 82/82 assertion-groups pass (77 prior + 5 net-new: 78-82; tests 35, 65, 67, 68, 73, 75, 76 rewritten in place to match the corrected contracts, per the same "supersede in place, correction note in the doc" convention used every prior round); all previously-listed adjacent ecosystem/generation/navigation/holonet/Allies-surface/Location/Job/Intel tests unaffected (`gm-faction-ecosystem-view-model`, `gm-faction-context-navigation-controller`, `gm-workspace-faction-contact-navigation-controller`, `gm-job-ecosystem-view-model`, `gm-job-source-location-identity`, `gm-job-context-navigation-controller`, `gm-intel-ecosystem-view-model`, `gm-intel-location-fact-identity`, `gm-intel-context-navigation-controller`, `gm-locations-ecosystem-view-model`, `gm-locations-context-navigation-controller`, `gm-locations-identity-and-hierarchy-integrity`, `gm-generation-phase8d3c-jobs-production` all pass); `node tools/run-rolling-tests.mjs` 195/195 pass; `node tools/run-rolling-syntax-check.mjs` 2459/2459 clean; `node tools/check-architecture-boundaries.mjs` unchanged at 37 pre-existing findings (none in touched files); `node tools/validate-partials.mjs` and `node tools/validate-data.js` clean; `system.json` parses.

**Correction to prior rounds' wording (rounds 8-9):** round 8's and round 9's correction-pass write-ups above, and the round-9 Final verdict, described a three-stage id resolution precedence — exact id, then unique case-insensitive legacy id, then name — as part of the shipped, final contract, including for the flexible `findFaction()`/`findFactionContact()` SEARCH helpers specifically. Round 10 removed the case-insensitive-id stage entirely (see finding 3 above): no real caller or persisted datum was ever found to depend on it, and this system has never shipped to a production world. Those sections are left as an unedited historical record of what shipped in each round; this note and round 10's own write-up above are authoritative for the current contract. Canonical ids are simply exact-match, case-sensitive, everywhere in this file now.

### Final verdict

**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all ten correction passes above. The §202-flagged SSOT gap is closed at every layer ten rounds of independent review checked — from the original name-matching bug at `upsertFaction()`/`upsertFactionContact()`, through every caller that could still resolve identity from display text or a permissive id-or-name match, down to the authority's own creation/update boundary, its id-minting collision-safety, the full precedence of its own flexible search helpers (name-vs-id, and refusing rather than guessing at every ambiguous stage of either), every Job-side consumer that builds a persisted draft's issuer identity from a structured or free-text Faction/Contact reference, the one remaining canonical-but-treated-as-free-text field (`Location.controllingFactionId`), and — round 10 — every structured name-only field that was still being tried as an id first, plus a reversed-burden-of-proof audit of the one remaining speculative compatibility seam. Round 10 also simplified the contract itself: canonical ids are now simple exact-match, case-sensitive comparisons everywhere in this file, with no case-insensitive-id compatibility stage (see round 10's addendum and finding 3 — this system has never shipped, so no real caller or persisted datum was ever found to justify that stage, and it was removed rather than propagated further). The full, final contract, organized by input-field type:

**Creation/update:**
- **NEW Faction/Contact:** caller supplies no canonical id → `FactionRegistryService` mints it, structurally collision-safe (`mintUniqueId()`, reserving every existing id — registry-global for Contacts)
- **EXISTING Faction/Contact update:** caller supplies a canonical id → the exact record must already exist, or the call fails
- **No exception:** no caller-chosen id is ever accepted for a record that doesn't yet exist, and no legacy-materialization seam was needed (none audited as necessary)

**Resolution, by field type:**
- **CANONICAL ID FIELD** (a field that is itself unambiguously canonical, whether alone or paired with an empty sibling name field — `Location.controllingFactionId`, a structured `factionId`/`contactId` from a rendered row, `deleteFactionContact`/`promoteFactionContactToActor`'s `factionId`/`contactId` params) → exact-id-only resolution (`resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`), case-sensitive; an unresolved id is a stale/missing reference, full stop — never rescued by a name search, never treated as ambiguous-among-candidates (there is nothing to be ambiguous about once no exact match exists)
- **EXPLICIT NAME FIELD** (a structured `factionName`/`contactName` field, genuinely separate from its sibling id field, used only once that id field is empty) → unique-name-only resolution (`resolveFactionByUniqueName()` / `resolveFactionContactByUniqueName()`, new in round 10), case-insensitive, never tried as an id — a Faction/Contact whose real canonical id happens to equal the intended record's display name can never steal the resolution
- **GENUINELY COMBINED id-or-name input** (one free-text string that may be either, kept only where a real current caller genuinely has just one such field) → the id-or-unique-name compatibility seam (`resolveFactionForMutation()` / `resolveFactionContactForMutation()`, the flexible `findFaction()`/`findFactionContact()` SEARCH helpers, and `FactionJobBridgeService`'s own `resolveFactionIdOrUniqueName()`/`resolveContactByIdThenName()`, the latter a thin delegate to the registry's own resolver rather than a second, subtly-different implementation) — exact canonical id first, case-insensitive display-name/slug match only if no exact id matched, refusing to guess among 2+ candidates at either stage
- a NON-EMPTY selector (of any of the three types above) that fails to resolve — not found, or ambiguous — always fails outright (`null`/throw, or soft-fail where the caller's own contract requires it); it is never silently substituted with a broader or different result (a Faction-only draft standing in for a requested Contact, a same-named record standing in for a stale id, an arbitrary pick among duplicates, a broken reference copied through as if it were resolved) — only a genuinely ABSENT selector may fall back to a broader intentional default
- display text never decides canonical sameness anywhere, an ambiguous name is refused rather than resolved to an arbitrary first match, and a real canonical id already in hand is always used authoritatively — resolving to nothing throws (or, in `applyScoreDelta()`'s batch-safe case, fails soft) rather than silently falling through to a fresh create, a name-matched substitute, a downgraded/broader result, or a phantom record under a caller-guessed id

**Compatibility posture (round 10 addendum, applies going forward):** this Faction/Contact registry contract has never shipped to a released, persisted user world. Compatibility/legacy-preservation behavior is kept only where a concrete, currently-real caller or persisted-data dependency requires it — the burden of proof is on justifying the seam, not on justifying its removal. `migrateLegacyIdentities()`'s recovery of a genuinely-missing legacy id's old effective identity (`slugify(name[-role])`) remains untouched, because it protects a real class of pre-hardening persisted data (round 2), not a speculative one; the case-insensitive-id compatibility stage rounds 8-9 added was removed (round 10) because no real caller or data ever needed it. Any future compatibility question in this system should be evaluated the same way.

Every legacy id, every duplicate-name case, the draft/canonical duality, and every audited cross-domain reference (Location, NPC, Job, Intel, Allies) remain stable across rename — and a legacy record's actual pre-hardening effective identity is preserved wherever it is unambiguous, so migrating an existing world cannot silently orphan a reference that already resolved correctly before this phase shipped. Migration is structurally collision-safe (reserved-id-pool minting) at both the healthy-id layer and the recovered-legacy-identity layer, globally unique for Contacts (not per-Faction), and reports genuine duplicate-id ambiguity rather than silently absorbing it. No Job/Location/Intel architecture was modified; no new canonical authority or generic framework was introduced. Stopping here for independent review, per standing practice — **not merged automatically, 8D-4 not started.**
