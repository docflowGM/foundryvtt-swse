...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all nine correction passes above. The §202-flagged SSOT gap is closed at every layer nine rounds of independent review checked — from the original name-matching bug at `upsertFaction()`/`upsertFactionContact()`, through every caller that could still resolve identity from display text or a permissive id-or-name match, down to the authority's own creation/update boundary, its id-minting collision-safety, the full precedence of its own flexible search helpers (name-vs-id, exact-vs-case-insensitive-id, and refusing rather than guessing at every ambiguous stage of either), every Job-side consumer that builds a persisted draft's issuer identity from a structured or free-text Faction/Contact reference, and the one remaining canonical-but-treated-as-free-text field (`Location.controllingFactionId`). The full, final contract:
...
**Resolution:**
- canonical id already known → exact-id-only resolution (`resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`), and any caller with genuinely separate id/name fields (not merely one combined string), OR a field that is itself unambiguously canonical even standing alone (`Location.controllingFactionId`), must resolve this way — never collapsing an id with a name field into one ambiguous string, and never routing a canonical-only field through a name-capable search helper
- a NON-EMPTY selector (id or name) that fails to resolve — not found, or ambiguous — always fails outright (`null`/throw, or soft-fail where the caller's own contract requires it); it is never silently substituted with a broader or different result (a Faction-only draft standing in for a requested Contact, a same-named record standing in for a stale id, an arbitrary pick among duplicates) — only a genuinely ABSENT selector may fall back to a broader intentional default
- legacy/free-text combined input (one string that may be an id or a name) → id-or-unique-name compatibility seam (`resolveFactionForMutation()` / `resolveFactionContactForMutation()`, the flexible `findFaction()`/`findFactionContact()` SEARCH helpers, and `FactionJobBridgeService`'s own `resolveFactionIdOrUniqueName()`/`resolveContactByIdThenName()`, the latter now a thin delegate to the registry's own resolver rather than a second, subtly-different implementation), intentionally permissive for their existing callers only — but every one of these tries a real EXACT canonical id first, a unique legacy case-insensitive id second (refusing outright on 2+ candidates, never falling through to name), and a case-insensitive display-name/slug match only last and only once, refusing to guess among 2+ candidates at any of those three stages
- display text never decides canonical sameness anywhere, an ambiguous name or id (case-exact or case-insensitive) is refused rather than resolved to an arbitrary first match, and a real canonical id already in hand is always used authoritatively — resolving to nothing throws (or, in `applyScoreDelta()`'s batch-safe case, fails soft) rather than silently falling through to a fresh create, a name-matched substitute, a downgraded/broader result, or a phantom record under a caller-guessed id
