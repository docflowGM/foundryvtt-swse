...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all five correction passes above. The §202-flagged SSOT gap is closed at every layer five rounds of independent review checked: canonical Faction/Contact identity is id-only (or unambiguous-name-only for the documented legacy/free-text compatibility seams, and never silently ambiguous) for every create/update/delete/read-feeding-a-mutation path across the FULL caller graph — not just `upsertFaction()`/`upsertFactionContact()` themselves, but every Actor-relationship mutator, the suggestion-approval flow, both UI-layer free-text callers (Job Board reusable contacts, Location Atlas lead reveal), and every remaining SEARCH-helper-feeds-a-mutation call site (`hide-contact`, both stages of the Intel bridge's Contact-linking resolution). The Job Board's Contact-resolution contract is now unconditional and final: `explicit factionId → exact Faction or FAIL (exact-id-only, never a name fallback)`; `no factionId → free-text Faction compatibility resolution (documented legacy/input seam, unique-name-or-create, unchanged)`; `explicit contactId → exact Contact under the resolved Faction or FAIL (exact-id-only, never a name fallback)`; `no contactId → CREATE NEW CONTACT`, always — a Contact name, tagged or not, never again decides canonical Contact sameness, and a genuinely canonical id already in hand can never be "rescued" by some other record's coincidentally-matching display name either. Display text never decides canonical sameness anywhere it previously did, an ambiguous name is refused rather than resolved to an arbitrary first match, and a real canonical id already in hand is always used authoritatively — resolving to nothing throws rather than silently falling through to a fresh create or a name-matched substitute. Every legacy id, every duplicate-name case, the draft/canonical duality, and every audited cross-domain reference (Location, NPC, Job, Intel, Allies) remain stable across rename — and a legacy record's actual pre-hardening effective identity is preserved wherever it is unambiguous, so migrating an existing world cannot silently orphan a reference that already resolved correctly before this phase shipped. Migration is structurally collision-safe (reserved-id-pool minting) at both the healthy-id layer and the recovered-legacy-identity layer, globally unique for Contacts (not per-Faction), and reports genuine duplicate-id ambiguity rather than silently absorbing it. No Job/Location/Intel architecture was modified; no new canonical authority or generic framework was introduced. Stopping here for independent review, per standing practice — **not merged automatically, 8D-4 not started.**
