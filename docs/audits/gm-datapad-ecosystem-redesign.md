...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all seven correction passes above. The §202-flagged SSOT gap is closed at every layer seven rounds of independent review checked — from the original name-matching bug at `upsertFaction()`/`upsertFactionContact()`, through every caller that could still resolve identity from display text or a permissive id-or-name match, down to the authority's own creation/update boundary (which can no longer mint a new canonical record under a caller-chosen id at all), its id-minting collision-safety, and the precedence of its own flexible search helpers. The full, final contract:
...
**Resolution:**
- canonical id already known → exact-id-only resolution (`resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`), and any caller with genuinely separate id/name fields (not merely one combined string) must resolve this way whenever the id field is populated, never collapsing the two into one ambiguous string first
- legacy/free-text combined input (one string that may be an id or a name) → id-or-unique-name compatibility seam (`resolveFactionForMutation()` / `resolveFactionContactForMutation()`, and the flexible `findFaction()`/`findFactionContact()` SEARCH helpers), intentionally permissive for their existing callers only — but even these always try a real canonical id across the WHOLE registry before ever trying a display-name/slug match, so an earlier record's name can never shadow a later record's real id
