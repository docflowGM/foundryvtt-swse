...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including both correction passes above. The §202-flagged SSOT gap is closed at every layer two rounds of independent review checked: canonical Faction/Contact identity is id-only (or unambiguous-name-only, and never silently ambiguous) for every create/update/delete/read-feeding-a-mutation path across the FULL caller graph — not just `upsertFaction()`/`upsertFactionContact()` themselves, but every Actor-relationship mutator, the suggestion-approval flow, the two UI-layer free-text callers (Job Board reusable contacts, Location Atlas lead reveal), the Job Board's own reuse-vs-overwrite distinction, and the two remaining SEARCH-helper-feeds-a-mutation call sites (`hide-contact`, the Intel bridge's Contact-linking resolution). Display text (name, role) never decides canonical sameness anywhere it previously did, and an ambiguous name is now refused rather than resolved to an arbitrary first match. Every legacy id, every duplicate-name case, the draft/canonical duality, and every audited cross-domain reference (Location, NPC, Job, Intel, Allies) remain stable across rename — AND, as of round 2, a legacy record's actual pre-hardening effective identity is preserved wherever it is unambiguous, so migrating an existing world cannot silently orphan a reference that already resolved correctly before this phase shipped. Migration is structurally collision-safe (reserved-id-pool minting) at both the healthy-id layer and the recovered-legacy-identity layer, globally unique for Contacts (not per-Faction), and reports genuine duplicate-id ambiguity rather than silently absorbing it. No Job/Location/Intel architecture was modified; no new canonical authority or generic framework was introduced. Stopping here for independent review, per standing practice — **not merged automatically, 8D-4 not started.**
