...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all six correction passes above. The §202-flagged SSOT gap is closed at every layer six rounds of independent review checked — from the original name-matching bug at `upsertFaction()`/`upsertFactionContact()`, through every caller that could still resolve identity from display text or a permissive id-or-name match, down to the authority's own creation/update boundary, which can no longer mint a new canonical record under a caller-chosen id at all. The full, final creation/update contract:
- **NEW Faction/Contact:** caller supplies no canonical id → `FactionRegistryService` mints it
...
- canonical id already known → exact-id-only resolution (`resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`)
- legacy/free-text combined input (one string that may be an id or a name) → id-or-unique-name compatibility seam (`resolveFactionForMutation()` / `resolveFactionContactForMutation()`), intentionally permissive for its existing callers only
