...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including the correction pass above. The §202-flagged SSOT gap is closed at every layer independent review checked: canonical Faction/Contact identity is id-only (or unambiguous-name-only, and never silently ambiguous) for every create/update/delete path across the FULL caller graph — not just `upsertFaction()`/`upsertFactionContact()` themselves, but every Actor-relationship mutator, the suggestion-approval flow, and the two UI-layer callers (Job Board reusable contacts, Location Atlas lead reveal) that independent review found were still exposed. Display text (name, role) never decides canonical sameness anywhere it previously did, and an ambiguous name is now refused rather than resolved to an arbitrary first match. Every legacy id, every duplicate-name case, the draft/canonical duality, and every audited cross-domain reference (Location, NPC, Job, Intel, Allies) remain stable across rename. Migration is now structurally collision-safe (reserved-id-pool minting), globally unique for Contacts (not per-Faction), and reports genuine duplicate-id ambiguity rather than silently absorbing it. No Job/Location/Intel architecture was modified; no new canonical authority or generic framework was introduced. Stopping here for independent review, per standing practice — **not merged automatically, 8D-4 not started.**
