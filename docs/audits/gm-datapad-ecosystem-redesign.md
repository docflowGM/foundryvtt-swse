...
**PRE-8D-4 FACTION / CONTACT CANONICAL IDENTITY HARDENING IS COMPLETE**, including all eight correction passes above. The §202-flagged SSOT gap is closed at every layer eight rounds of independent review checked — from the original name-matching bug at `upsertFaction()`/`upsertFactionContact()`, through every caller that could still resolve identity from display text or a permissive id-or-name match, down to the authority's own creation/update boundary, its id-minting collision-safety, the precedence of its own flexible search helpers (both name-vs-id and exact-vs-case-insensitive-id), and every Job-side consumer that builds a persisted draft's issuer identity from a structured or free-text Faction/Contact reference. The full, final contract:
...
**Resolution:**
- canonical id already known → exact-id-only resolution (`resolveFactionByIdForMutation()` / `resolveFactionContactByIdsForMutation()`), and any caller with genuinely separate id/name fields (not merely one combined string) must resolve this way whenever the id field is populated, never collapsing the two into one ambiguous string first — including a caller's own STRUCTURED filter/draft-building seam (`buildDraftFromIssuerFilter()`), not merely direct mutation calls
- legacy/free-text combined input (one string that may be an id or a name) → id-or-unique-name compatibility seam (`resolveFactionForMutation()` / `resolveFactionContactForMutation()`, and the flexible `findFaction()`/`findFactionContact()` SEARCH helpers, plus `FactionJobBridgeService`'s own `resolveFactionIdOrUniqueName()`/`resolveContactByIdThenName()`), intentionally permissive for their existing callers only — but every one of these tries a real EXACT canonical id first, a unique legacy case-insensitive id second, and a display-name/slug match only last and only once, refusing to guess among 2+ candidates at any of those three stages
- display text never decides canonical sameness anywhere, an ambiguous name (or ambiguous case-insensitive id) is refused rather than resolved to an arbitrary first match, and a real canonical id already in hand is always used authoritatively — resolving to nothing throws (or, in `applyScoreDelta()`'s batch-safe case, fails soft) rather than silently falling through to a fresh create, a name-matched substitute, or a phantom record under a caller-guessed id
