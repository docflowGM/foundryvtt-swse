...
| Autofire Assault | fully_implemented | Normalized as an ACTIVE autofire `ATTACK_OPTION` gated by an autofire-capable/current-autofire weapon and matching owned Weapon Focus choice. When selected, it reduces the expected normal autofire penalty from -5 to -2 by applying a +3 attack adjustment and adds +1 weapon die. |
| Autofire Sweep | implemented_as_area_metadata | Normalized as an ACTIVE autofire `ATTACK_OPTION` gated by autofire and matching owned Weapon Focus choice. When selected, it emits 6-square cone area metadata from a visible point in point-blank range; target selection/area placement remains GM/workflow adjudicated. |
