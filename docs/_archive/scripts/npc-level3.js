...
Hooks.on('ready', () => {
  Hooks.on('renderApplicationV2', (app) => {
    ...
    if (role) {
      if (role === 'minion') {level = 'minion';} else if (role === 'elite') {level = 'elite';} else if (role === 'boss') {level = 'boss';}
    } else {
      if (cr <= 3) {level = 'minion';} else if (cr <= 8) {level = 'standard';} else if (cr <= 14) {level = 'elite';} else {level = 'boss';}
    }
