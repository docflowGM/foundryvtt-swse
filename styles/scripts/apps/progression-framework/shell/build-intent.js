...
export class BuildIntent {
  ...
  commitSelection(stepId, selectionKey, value) {
    ...
    try {
      ...
      const currentValue = session?.getSelection?.(canonicalKey);
      const alreadyCommitted = this._deepEqual(currentValue, value);

      // StepPluginBase commits to ProgressionSession before touching this legacy
      // facade. Avoid re-entering the canonical session with the same payload; the
      // double commit caused duplicate persistence/render passes and made legacy
      // singleton feat/talent payloads capable of tripping schema validation.
      const success = alreadyCommitted
        ? true
        : session?.commitSelection(stepId, canonicalKey, value);
