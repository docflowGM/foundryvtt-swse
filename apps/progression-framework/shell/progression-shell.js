...
export class ProgressionShell extends SWSEApplicationV2 {
  ...
  requestRender(reason = 'unspecified', ...args) {
    ...
    if (args.length) this._lastRenderArgs = args;
    if (this._isRendering) {
      console.warn(`[ProgressionShell] ⚠️ Render requested while already rendering — QUEUED (${reason})`);
      return this;
    }
    const renderArgs = this._lastRenderArgs ?? args;
    this._pendingRender = false;
    this._lastRenderArgs = null;
  ...
  async render(...args) {
    // Render loop prevention: queue recursive render calls during active render
    if (this._isRendering) {
      this._pendingRender = true;
      this._lastRenderArgs = args;
      console.warn("[ProgressionShell] ⚠️ Render called while already rendering — QUEUED (loop prevention)");
      return this;
    }
    ...
    if (this._pendingRender) {
      const queuedArgs = this._lastRenderArgs ?? [];
      this._pendingRender = false;
      this._lastRenderArgs = null;
  ...
  async _prepareContext(options) {
    ...
    const focusedItemSnapshot = this.focusedItem ? foundry.utils.deepClone(this.focusedItem) : this.focusedItem;
    const detailsPanelSpec = currentPlugin?.renderDetailsPanel(focusedItemSnapshot)
      ?? { template: null, data: {} };
    ...
    ProgressionDebugCapture.log('Progression Debug', `[Render #${renderNum}] Template HTML rendered`, {
      ...
      focusedItem_name_in_html: focusedItemSnapshot?.name ? detailsPanelHtml?.includes?.(focusedItemSnapshot.name) : 'N/A',
    });
    ...
    ProgressionDebugCapture.detectStateDrift(
      ...
      { focusedItem: focusedItemSnapshot?.name }
    );

    // Species-specific: Detect details panel hydration failure
    diagnostics.detectSpeciesDetailsPanelFailure(currentDescriptor?.stepId, focusedItemSnapshot, detailsPanelHtml);
