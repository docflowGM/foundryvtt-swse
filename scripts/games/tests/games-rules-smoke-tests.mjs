...
import { scorePazaakPlayer, hasFilledPazaakTable, comparePazaakSet } from '../games/pazaak/pazaak-rules.js';
...
function testPazaak() {
  ...
  const tiedWithLastTiebreaker = comparePazaakSet([
    { seatId: 'early', tableCards: [{ value: 10 }, { value: 9 }, { value: 1, tiebreaker: true }], tiebreakerUsed: true, tiebreakerPlayedAt: 1 },
    { seatId: 'late', tableCards: [{ value: 10 }, { value: 9 }, { value: 1, tiebreaker: true }], tiebreakerUsed: true, tiebreakerPlayedAt: 2 }
  ]);
  assert.equal(tiedWithLastTiebreaker.winnerSeatId, 'late', 'Last played Pazaak tiebreaker should win tied totals');
}
...
function testSabacc() {
  ...
  const idiotsArray = evaluateSabaccHand([buildSabaccSylopCard(0), buildSabaccNumberCard(suit, 2), buildSabaccNumberCard(SABACC_SUITS[1], 3)]);
  assert.equal(idiotsArray.handType, 'idiots-array', "Idiot's Array should be recognized as Sylop + 2 + 3 even when total is not zero");
  const specialResult = compareSabaccHands([
    { seatId: 'idiot', cards: [buildSabaccSylopCard(0), buildSabaccNumberCard(suit, 2), buildSabaccNumberCard(SABACC_SUITS[1], 3)] },
    { seatId: 'pure', cards: [buildSabaccSylopCard(0), buildSabaccSylopCard(1)] }
  ]);
  assert.equal(specialResult.winnerSeatId, 'idiot', "Idiot's Array should outrank Pure Sabacc");
...
function testHintaro() {
  ...
  assert.equal(rolled.length, 4, 'Hintaro regular dice should expose four visible symbols');
