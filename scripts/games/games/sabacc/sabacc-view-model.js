...
export class SabaccViewModel {
  static build({ session, actor, participantId } = {}) {
    ...
    return {
      ...
      marketRuleLabel: state.marketRuleLabel || 'Holopad Casino Market — house-rule buy/trade option',
      jackpotRuleLabel: state.jackpotRuleLabel || "Sabacc pot pays only on Idiot's Array or Pure Sabacc.",
      phasePlan: state.phasePlan || 'Card action → betting → spike dice, repeated for three rounds.',
