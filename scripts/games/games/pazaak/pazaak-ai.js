...
export const PAZAAK_AI_DIFFICULTIES = Object.freeze({
  easy: { id: 'easy', label: 'Easy', standAt: 16, pushAt: 14, bustFixOnlyAt: 21, mistakeRate: 0.28, conserveCards: 0.1, thinkingDelayMs: 5000 },
  medium: { id: 'medium', label: 'Medium', standAt: 18, pushAt: 16, bustFixOnlyAt: 21, mistakeRate: 0.12, conserveCards: 0.35, thinkingDelayMs: 4000 },
  hard: { id: 'hard', label: 'Hard', standAt: 19, pushAt: 17, bustFixOnlyAt: 21, mistakeRate: 0.05, conserveCards: 0.55, thinkingDelayMs: 3000 },
  pro: { id: 'pro', label: 'Pro', standAt: 19, pushAt: 18, bustFixOnlyAt: 21, mistakeRate: 0.015, conserveCards: 0.75, thinkingDelayMs: 2000 },
  grandmaster: { id: 'grandmaster', label: 'Grandmaster', standAt: 20, pushAt: 18, bustFixOnlyAt: 21, mistakeRate: 0, conserveCards: 0.9, thinkingDelayMs: 1000 }
});
...
export function buildPazaakAiProfile(profile = {}) {
  if (typeof profile === 'string') {
    ...
    return { difficulty: normalizeDifficulty(legacyMap[profile] || profile), fairness: 'fair', personality, personalityLabel: personalityData.label, personalityQuality: personalityData.quality, forceSensitive: false, forceSensitivityChance: 0.05 };
  }
  ...
  const personalityData = resolvePazaakAiPersonality(personality);
  return { ...profile, difficulty, fairness, personality, personalityLabel: profile.personalityLabel || personalityData.label, personalityQuality: profile.personalityQuality || personalityData.quality, forceSensitive: Boolean(profile.forceSensitive), forceSensitivityChance: Number(profile.forceSensitivityChance ?? 0.05) || 0.05, gmControlled: Boolean(profile.gmControlled || fairness === 'gmControlled') };
}
...
export class PazaakAi {
  static chooseTurn(player = {}, rawProfile = 'medium', context = {}) {
    ...
    if (profile.forceSensitive && nextMainCard && Math.random() < Math.max(0, Math.min(1, profile.forceSensitivityChance))) {
      ...
      if (feeling === 'dangerous' && score >= Math.max(14, standAt - 2) && score <= PAZAAK_TARGET) return { type: 'stand', forceIntuition: { feeling, line } };
      if (feeling === 'excellent' && score < PAZAAK_TARGET) return { type: 'end-turn', forceIntuition: { feeling, line } };
    }
