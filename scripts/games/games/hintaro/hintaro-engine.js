...
import { evaluateHintaroRoll, rollHintaroDie, rollHintaroRegularDice, rollHintaroRegularDie, compareHintaroEvaluations } from './hintaro-rules.js';
...
function updateEvaluations(state) {
  for (const player of Object.values(state.players ?? {})) player.evaluation = evaluateHintaroRoll(player.symbols || [], state.hintaroDie);
}
...
function beginRound(session, state) {
  ...
  pushEvent(session, state, 'round-start', state.hintaronSeatId, `Round ${state.round} begins. ${seatLabel(session, state.hintaronSeatId)} is hintaron.`, { tone: 'credits' });
...
function expectedRankForDie(player, index) {
  const faces = ['tukar', 'kulro'];
  let total = 0;
  for (const face of faces) {
    const symbols = [...(player.symbols || [])];
    symbols[index] = face;
...
function chooseAiReroll(player) {
  ...
  for (let i = 0; i < (player.symbols || []).length; i += 1) {
...
function applyRerollAction(session, state, seat, action, payload = {}) {
  ...
  if (action === 'reroll-die') {
    const index = Math.max(0, Math.min((player.symbols || []).length - 1, Math.floor(Number(payload.dieIndex ?? 0))));
    player.symbols[index] = rollHintaroRegularDie();
    player.rerolled = true;
    player.lastAction = `Rerolls cube ${index + 1}.`;
    player.evaluation = evaluateHintaroRoll(player.symbols, null);
...
function resolveRound(session, state) {
  ...
  if (!winners.length) {
    ...
    state.pot = 0;
    state.message = winners.length > 1 ? `The round ties; ${share} credits split between winners.` : `${seatLabel(session, winners[0])} wins the Hintaro pot.`;
    pushEvent(session, state, 'round-winner', winners[0] || null, state.message, { tone: 'success', winners });
  }
...
async function processAi(session, state) {
  ...
  while (guard < 20) {
    ...
    if (state.phase === 'betting') {
      ...
      const paid = safeAmount(state.betting?.contributions?.[seat.seatId], 0);
      const toCall = Math.max(0, currentBet - paid);
      let action = toCall > 0 ? (rank <= 0 && toCall > state.ante ? 'drop' : 'call-bet') : 'check';
      if (toCall <= 0 && rank >= 3 && currentBet <= 0) action = 'bet';
      const result = applyBettingAction(session, state, seat, action, { amount: state.betting?.minBet || state.ante });
      if (!result.ok) break;
      continue;
    }
...
export class HintaroEngine {
  ...
  static async createSoloAiSession({ actor, actorId = null, title = '', sessionId = null, requesterId = null, rulesMode = 'republic-senate', creditBuyIn = 0 } = {}) {
    ...
    const generated = await GameOpponentProfileService.buildPazaakAiOpponentProfile({ difficulty: settings.defaultAiDifficulty || 'medium', fairness: 'fair', personality: settings.defaultAiPersonality || 'random' });
    const aiProfile = { ...generated, difficulty: settings.defaultAiDifficulty || 'medium', fairness: 'fair', personality: generated.personality || 'methodical' };
