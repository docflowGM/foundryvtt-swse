...
export class SWSEActor extends Actor {
  ...
  _ensureSystemStructure() {
    ...
    if (!sys.xp) sys.xp = { value: 0, required: 1000 };
...
export class SWSEActorSheet extends ActorSheet {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["swse", "sheet", "actor", "character", "swse-v2-sheet", "holo-theme"],
  ...
  getData() {
    ...
    const skills = Object.entries(system.skills || {}).map(([key, skill]) => ({
      key,
      label: skill.label || key.toUpperCase(),
      ability: skill.ability || 'str',
      trained: !!skill.trained,
      focus: !!skill.focus,
      value: this.actor.getSkillMod(skill)
    }));

    const defenses = [
      { key: 'reflex', label: 'REF', total: system.defenses?.reflex?.total ?? 10 },
      { key: 'fortitude', label: 'FORT', total: system.defenses?.fortitude?.total ?? 10 },
      { key: 'will', label: 'WILL', total: system.defenses?.will?.total ?? 10 },
      { key: 'dt', label: 'DT', total: system.damageThreshold ?? 10 }
    ];

    context.system = system;
    context.items = items;
    context.damageThreshold = system.damageThreshold;
    context.races = SWSE_RACES;
    context.labels = {
      sheetTitle: game.i18n.localize("SWSE.SheetLabel.character") || "Character"
    };
    context.abilitiesList = Object.entries(system.abilities || {}).map(([key, ability]) => ({
      key,
  ...
  activateListeners(html) {
    ...
    html.find(".level-up, .app-training, [data-action='level-up']").click(this._onLevelUp.bind(this));
    html.find(".apply-second-wind").click(this._onSecondWind.bind(this));
    html.find(".app-workbench").click(() => this._activateTabByName('gear'));
    html.find(".app-character").click(() => this._activateTabByName('overview'));
    html.find(".app-store, [data-action='cmd-store']").click(this._openStore.bind(this));
    html.find(".app-progress, [data-action='cmd-chargen']").click(this._launchProgression.bind(this));
  }

  _activateTabByName(tabName) {
    const nav = this.element?.find(`.sheet-tabs [data-tab="${tabName}"]`);
    nav?.trigger('click');
  }

  async _openStore(event) {
    event?.preventDefault?.();
    if (game.swse?.openStore) {
      game.swse.openStore(this.actor);
      return;
    }
    const module = await import('../store/store.js').catch(() => null);
    if (module?.SWSEStore) new module.SWSEStore(this.actor).render(true);
  }

  async _launchProgression(event) {
    event?.preventDefault?.();
    try {
      const module = await import('./apps/progression-framework/progression-entry.js');
      if (module?.launchProgression) return module.launchProgression(this.actor);
    } catch (err) {
      console.warn('SWSE | Progression launcher unavailable, falling back to level up.', err);
    }
  ...
  async _onAddForcePower(event) { event.preventDefault(); await this.actor.createEmbeddedDocuments("Item", [{ name: "New Force Power", type: "force-power", system: { uses: { current: 1, max: 1 } } }]); }
