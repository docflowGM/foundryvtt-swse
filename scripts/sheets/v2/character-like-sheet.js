...
export class SWSEV2CharacterLikeSheet extends SWSEV2ActorSheetBase {
  ...
  async _prepareContextForActorSheet({
    ...
    if (useNpcConceptSheet) {
      // NPC-exclusive concept abilities/profile context assembly lives on
      // SWSEV2NpcSheet (see _buildNpcConceptAbilitiesContext there); this
      // base class provides only the no-op default below so the call site
      // stays correct for Character/Droid instances too.
      this._buildNpcConceptAbilitiesContext(context, actor);
    }
...
const forcePoints = [];
  ...
  /**
   * NPC-exclusive concept abilities/profile context assembly. No-op default
   * so the shared _prepareContextForActorSheet call site (behind an
   * `if (useNpcConceptSheet)` guard that is only ever true for actual NPC
   * instances) is correct for every subtype. Overridden by SWSEV2NpcSheet
   * with the original inline block's exact body.
   */
