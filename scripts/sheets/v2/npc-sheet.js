...
/**
 * SWSEV2NpcSheet - NPC Sheet Application
 *
 ...
 */
export class SWSEV2NpcSheet extends HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
  static PARTS = {
    ...super.PARTS,
    body: {
  ...
  constructor(document, options = {}) {
    ...
    if (!this.constructor.DEFAULT_OPTIONS?.template) {
      throw new Error(
        'SWSEV2NpcSheet: Missing template path in DEFAULT_OPTIONS. Sheet cannot render.'
      );
    }
  }

  async _prepareContext(options) {
    RenderAssertions.logCheckpoint('NpcSheet', 'prepareContext start');
    ...
    RenderAssertions.assertContextSerializable(overrides, 'SWSEV2NpcSheet');
    RenderAssertions.logCheckpoint('NpcSheet', 'prepareContext complete', { actorId: actor.id });
  ...
  async _onRender(_context, _options) {
    RenderAssertions.logCheckpoint('NpcSheet', '_onRender start');

    // AppV2 invariant: all DOM access must use this.element
    const root = this.element;
    if (!(root instanceof HTMLElement)) {
      throw new Error('NpcSheet: element is not an HTMLElement after render');
    }
    ...
    if (!root.querySelector('.sheet-body')) {
      RenderAssertions.logCheckpoint('NpcSheet', '_onRender: skipping assertion (partial PARTS render)');
      return;
    }
    ...
    RenderAssertions.assertRenderComplete(this, 'SWSEV2NpcSheet');
  }
