/**
 * SWSE Force Secret Suggestion Engine
 *
...
export class ForceSecretSuggestionEngine {
   ...
   * @param {Object} characterData - Pending character data (for chargen)
   * @param {Object} options - Additional options
   * @returns {Promise<Array>} Filtered suggestions (only tier 3+)
   */
  static async suggestForceSecrets(availableSecrets = [], actor = null, characterData = {}, options = {}) {
    if (!availableSecrets || availableSecrets.length === 0) {
      return [];
    }

    const knownPowers = this._getKnownForcePowers(actor, characterData);
    const knownTechniques = this._getKnownForceTechniques(actor, characterData);
    const archetype = this._getCharacterArchetype(actor, characterData);
    const institution = this._getInstitution(actor, characterData);
   ...
   * @private
   */
  static _getKnownForcePowers(actor = null, characterData = {}) {
    const powers = [];
   ...
   * @private
   */
  static _getKnownForceTechniques(actor = null, characterData = {}) {
    const techniques = [];
    ...
    if (characterData.feats && Array.isArray(characterData.feats)) {
      ...
      characterData.feats.forEach(feat => {
        ...
        if (featName && !techniques.includes(featName)) {
          techniques.push(featName);
        }
      });
    }
   ...
   * @private
   */
  static _getInstitution(actor = null, characterData = {}) {
    const dsp = actor?.system?.swse?.darkSidePoints || characterData.darkSidePoints || 0;
    const maxDSP = actor?.system?.swse?.maxDarkSidePoints || characterData.maxDarkSidePoints || 1;

    // Check explicit institution
    const explicit = actor?.system?.swse?.institution || characterData.institution;
    if (explicit) {
...
export default ForceSecretSuggestionEngine;
