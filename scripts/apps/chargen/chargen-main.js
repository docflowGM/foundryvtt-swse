...
export default class CharacterGenerator extends SWSEApplicationV2 {
  ...
  /**
   * Random living-being names and droid designations are data-driven via
   * data/chargen/random-names.json. Use SharedModule.getRandomName() and
   * SharedModule.getRandomDroidName() instead of embedding name pools here.
   */
  ...
  async _onRandomName(event) {
    ...
    const selectedName = await SharedModule.getRandomName?.(this.actor);
    if (!selectedName) {
  ...
  async _onRandomDroidName(event) {
    ...
    const selectedName = await SharedModule.getRandomDroidName?.(this.actor);
    if (!selectedName) {
  ...
  async _updateActor() {
    ...
    // Write canonical current HP; recomputeHP will recalculate system.hp.max from level+CON formula.
    updates['system.hp.value'] = (this.actor.system.hp?.value || 0) + hpGain;

    await globalThis.SWSE.ActorEngine.updateActor(this.actor, updates);
    // Recalculate HP max from the new level (reads system.level, writes system.hp.max)
    await globalThis.SWSE.ActorEngine.recomputeHP(this.actor);
