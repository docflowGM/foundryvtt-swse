...
export class ProjectionEngine {
  ...
  static _projectAttributes(draftSelections, actor) {
    ...
    const readScore = (key) => {
      const selected = attrSelection?.values?.[key] ?? attrSelection?.[key];
      if (Number.isFinite(Number(selected))) {
        return Number(selected);
      }

      const actorValue = actorAbilities?.[key]?.base
        ?? actorAbilities?.[key]?.value
        ?? actorAbilities?.[key]?.total
        ?? actorAttributes?.[key]?.value
        ?? actorAttributes?.[key]?.base
        ?? 10;

      return Number(actorValue) || 10;
    };
    ...
    const species = ProgressionContentAuthority.resolveSpecies(draftSelections.species);
    const speciesAbilityScores = species?.abilityScores || species?.system?.abilityScores || {};
    const speciesKeyMap = {
      strength: 'str',
      dexterity: 'dex',
      constitution: 'con',
    ...
    for (const [ability, mod] of Object.entries(speciesAbilityScores)) {
      const normalizedKey = speciesKeyMap[String(ability || '').toLowerCase()];
      if (normalizedKey && normalizedKey in scores) {
        scores[normalizedKey] += Number(mod || 0);
      }
    }
  ...
  static _projectAbilities(draftSelections, actor = null, mode = 'chargen') {
    ...
    return {
      ...
      forceRegimens: ProgressionContentAuthority.normalizeSelectionList('forceRegimen', draftSelections.forceRegimens),
  ...
  static _buildEmptyProjection() {
    return {
      ...
      abilities: { feats: [], talents: [], forcePowers: [], forceRegimens: [], forceTechniques: [], forceSecrets: [], starshipManeuvers: [] },
      languages: [],
      droid: null,
