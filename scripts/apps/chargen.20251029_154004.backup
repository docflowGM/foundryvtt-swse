...
export default class CharacterGenerator extends Application {
  ...
  _bindAbilitiesUI(root) {
    ...
    const rollStandard = () => {
      ...
      if (container) {
        ...
        assets/ui.notifications.info("Standard rolls generated — click a result then click an ability to assign.");
      }
    };
    ...
    const rollOrganic = () => {
      ...
      if (container) {
        ...
        assets/ui.notifications.info("Organic roll completed — click a group, then click an ability to assign.");
      }
    ...
    for (const skill of this._skillsJson) {
      ...
      trainedCheck.onchange = (ev) => {
        if (ev.target.checked && trainedCount >= maxTrained) {
          assets/ui.notifications.warn(`Maximum trained skills (${maxTrained}) reached!`);
  ...
  async _createActor() {
    // Bassets/uild proper actor data structure matching SWSEActorSheet expectations
    const system = {
    ...
    try {
      ...
      assets/ui.notifications.info(`Character ${this.characterData.name} created successfully!`);
    } catch (err) {
  ...
  async _updateActor() {
    ...
    assets/ui.notifications.info(`${this.actor.name} leveled up to level ${newLevel}!`);
  }
}