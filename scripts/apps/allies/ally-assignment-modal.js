...
export class AllyAssignmentModal extends SWSEApplicationV2 {
  ...
  async _prepareContext(options) {
    ...
    const viewModel = AlliesSurfaceService.buildNpcAssignmentPickerViewModel(this.ownerActor);
  ...
  _onRender(context, options) {
    ...
    root.querySelectorAll('[data-button="confirm"]').forEach(button => {
  ...
  _onSelectFollowerSlot(slotId) {
    this.state = { ...this.state, followerSlotId: slotId };
    this.render(true);
  }
