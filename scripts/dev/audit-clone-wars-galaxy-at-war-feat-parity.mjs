...
const MANIFEST_PATH = path.join(ROOT, 'data/feat-source-parity/clone-wars-galaxy-at-war-feat-parity-manifest.json');
...
const JSON_OUT = path.join(OUT_DIR, 'clone-wars-galaxy-at-war-feat-parity-report.json');
const MD_OUT = path.join(OUT_DIR, 'clone-wars-galaxy-at-war-feat-parity-report.md');
...
let cloneWarsCount = 0;
let galaxyAtWarCount = 0;
let runtimeClassifiedCount = 0;
let selectedChoiceCount = 0;
let notForceReviewedCount = 0;
let forceContextOnlyCount = 0;
let combatOptionCount = 0;
let vehicleContextCount = 0;
let skillContextCount = 0;
...
for (const entry of manifest.expectedFeats ?? []) {
  ...
  if (new RegExp(entry.sourcebook.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(source)) {
    ...
    if (/clone wars/i.test(source)) cloneWarsCount += 1;
    if (/galaxy at war/i.test(source)) galaxyAtWarCount += 1;
  } else {
  ...
  const runtimeMarker = meta.requiresRuntimeContext === true
    ...
    || includesAny(`${mode} ${scope} ${policy} ${status} ${bucket} ${implementationHome}`, [/context/i, /metadata/i, /conditional/i, /runtime/i, /reaction/i, /resource/i, /choice/i, /option/i, /rider/i, /encounter/i, /vehicle/i, /autofire/i]);
  ...
  if (/combat|attack|rider|area/i.test(implementationHome)) combatOptionCount += 1;
  if (/vehicle|gunnery/i.test(implementationHome)) vehicleContextCount += 1;
  if (/skill|healing|repair/i.test(implementationHome)) skillContextCount += 1;
  ...
  if (entry.requiresSelectedChoice && meta.requiresSelectedChoice !== true) {
    const selectionContext = /choice|picker|selected|selection|entitlement|exclude|target/i.test(`${mode} ${scope} ${policy} ${status} ${bucket}`);
    if (!selectionContext) {
  ...
  if (riskyStatic) {
    findings.push(finding('warning', 'UNCONDITIONAL_STATIC_MODIFIER_RISK', `${name} has an enabled unconditional numeric modifier. Verify it is not escaping its Clone Wars/Galaxy at War context.`, { name, modifier: riskyStatic }));
  }
  ...
  if (entry.forceContextOnly) {
    const featType = String(feat?.system?.featType ?? '');
    if (/^force$/i.test(featType)) {
      findings.push(finding('error', 'FORCE_CONTEXT_ONLY_TYPED_AS_FORCE', `${name} is force-adjacent but should remain a general feat unless source prerequisites make it a true Force feat.`, { name, featType, tags }));
    } else {
      forceContextOnlyCount += 1;
    }
  }

  if (/Force/.test(name) && !entry.forceFeat && !entry.notForceFeat && !entry.forceContextOnly && !entry.titleKeywordReviewed) {
    findings.push(finding('warning', 'FORCE_NAME_REQUIRES_REVIEW', `${name} contains Force in the title but is not explicitly reviewed in the manifest. Confirm classification by source context.`, { name, classification: entry.classification }));
  }
}
...
const cloneWarsCatalogCount = catalog.filter((feat) => /clone wars/i.test(getSource(feat))).length;
const galaxyAtWarCatalogCount = catalog.filter((feat) => /galaxy at war/i.test(getSource(feat))).length;
...
const report = {
  ...
  cloneWarsCount,
  galaxyAtWarCount,
  cloneWarsCatalogCount,
  galaxyAtWarCatalogCount,
  runtimeClassifiedCount,
  selectedChoiceCount,
  notForceReviewedCount,
  forceContextOnlyCount,
  combatOptionCount,
  vehicleContextCount,
  skillContextCount,
...
function writeReports() {
  ...
  const lines = [
    ...
    `- Expected Clone Wars/Galaxy at War feats: ${report.expectedFeatCount}`,
    `- Present expected feats: ${report.presentExpectedFeatCount}`,
    `- Source-matched expected feats: ${report.sourceMatchedCount}`,
    `- Clone Wars expected feats matched: ${report.cloneWarsCount}`,
    `- Galaxy at War expected feats matched: ${report.galaxyAtWarCount}`,
    `- Runtime/context-classified feats: ${report.runtimeClassifiedCount}`,
    `- Combat/attack-option classified feats: ${report.combatOptionCount}`,
    `- Vehicle/gunnery-context feats: ${report.vehicleContextCount}`,
    `- Skill/healing/repair-context feats: ${report.skillContextCount}`,
    `- Not-Force title keyword reviews: ${report.notForceReviewedCount}`,
    `- Force-context-only reviews: ${report.forceContextOnlyCount}`,
  ...
  lines.push('', '## Reviewed Force-word / Force-adjacent cases', '');
  for (const entry of manifest.expectedFeats ?? []) {
    if (entry.notForceFeat || entry.forceContextOnly || /Force/.test(entry.name)) {
      lines.push(`- **${entry.name}:** ${entry.notes ?? entry.classification}`);
    }
  }
...
console.log(`Clone Wars/Galaxy at War feat parity audit: ${presentExpectedFeatCount}/${manifest.expectedFeats?.length ?? 0} expected feats present, ${warningCount} warnings, ${errorCount} errors`);
