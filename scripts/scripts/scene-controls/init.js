...
export function initializeSceneControls() {
  ...
  Hooks.on('getSceneControlButtons', (controls) => {
    ...
    if (!Array.isArray(groups)) {
      console.warn('[SWSE Scene Controls] Unexpected getSceneControlButtons payload', controls);
      return;
    }

    const swseControls = sceneControlRegistry.getControls();
    const existing = new Set(groups.map(group => group?.name).filter(Boolean));
    for (const control of swseControls) {
      if (existing.has(control?.name)) continue;
      groups.push(control);
    }
  });
}
