...
export async function handleFormSubmission(sheet, event) {
  ...
  const formElement = form instanceof HTMLFormElement
    ? form
    : (form?.querySelector?.('form') ?? null);
  const container = formElement ?? form;

  // Collect FormData
  let formData;
  try {
    formData = formElement ? new FormData(formElement) : new FormData();

    if (!formElement && container?.querySelectorAll) {
      for (const field of container.querySelectorAll('input[name], textarea[name], select[name]')) {
    ...
    for (const checkbox of container?.querySelectorAll?.('input[type="checkbox"][name]') ?? []) {
