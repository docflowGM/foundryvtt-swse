...
export function ShellHostMixin(BaseClass) {
  return class ShellHostMixinClass extends BaseClass {
    ...
    _wireHomeSurfaceEvents(root) {
      ...
      homeRoot.querySelectorAll('[data-holonet-record-id], [data-holonet-thread-id]').forEach(el => {
        el.addEventListener('click', async (ev) => {
          ev.preventDefault();
          const threadId = el.dataset.holonetThreadId;
          if (threadId) {
            await this.setSurface('messenger', { threadId, source: 'home-feed' });
            this.render(false);
            return;
          }
    ...
    async _wireMessengerSurfaceEvents(root) {
      ...
      messengerRoot.querySelectorAll('[data-shell-action="messenger-compose"]').forEach(el => {
        el.addEventListener('click', async (ev) => {
          ev.preventDefault();
      ...
      messengerRoot.querySelectorAll('[data-shell-action="messenger-cancel-compose"]').forEach(el => {
        el.addEventListener('click', async (ev) => {
          ev.preventDefault();
      ...
      this._wireMessengerRecipientFilters(messengerRoot);
      ...
      messengerRoot.querySelectorAll('form[data-holonet-action="send-message"]').forEach(form => {
        form.addEventListener('submit', async (ev) => {
          ...
          form.reset();
          await this.setSurface('messenger', threadId ? { threadId, source: 'messenger' } : { source: 'messenger' });
          this.render(false);
        });
      });
    }

    _wireMessengerRecipientFilters(messengerRoot) {
      const search = messengerRoot.querySelector('[data-contact-search]');
      const filterButtons = Array.from(messengerRoot.querySelectorAll('[data-contact-filter]'));
      const options = Array.from(messengerRoot.querySelectorAll('[data-contact-option]'));
      const groups = Array.from(messengerRoot.querySelectorAll('[data-contact-group]'));
      ...
      const apply = () => {
        ...
        for (const option of options) {
          const label = String(option.dataset.contactLabel ?? '').toLowerCase();
          const category = option.dataset.contactCategory ?? 'player';
          const matchesFilter = activeFilter === 'all' || activeFilter === category;
          const matchesQuery = !query || label.includes(query);
          option.hidden = !(matchesFilter && matchesQuery);
        }
        for (const group of groups) {
          const visible = Array.from(group.querySelectorAll('[data-contact-option]')).some(option => !option.hidden);
      ...
      filterButtons.forEach(button => {
        button.addEventListener('click', ev => {
          ev.preventDefault();
          activeFilter = button.dataset.contactFilter || 'all';
