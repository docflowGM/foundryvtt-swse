    ...
    async _wireMessengerSurfaceEvents(root) {
      ...
      messengerRoot.querySelectorAll('form[data-holonet-action="send-message"]').forEach(form => {
        form.addEventListener('submit', async (ev) => {
          ...
          await HolonetMessengerService.sendMessage({ actor, body, imageUrl, threadId, recipientIds });
          form.reset();
          this.render(false);
        });
      });
      ...
      messengerRoot.querySelectorAll('form[data-holonet-action="create-thread"]').forEach(form => {
        form.addEventListener('submit', async (ev) => {
          ...
          const recipientIds = data.getAll('recipientIds').map(String).filter(Boolean);
          await HolonetMessengerService.createThread({ actor, body, title, threadType, recipientIds, imageUrl });
          form.reset();
          await this.setSurface('messenger', { source: 'messenger' });
          this.render(false);
        });
      ...
      messengerRoot.querySelectorAll('form[data-holonet-action="create-job"]').forEach(form => {
        form.addEventListener('submit', async (ev) => {
          ...
          const recipientIds = data.getAll('recipientIds').map(String).filter(Boolean);
          await HolonetMessengerService.createJobPosting({ actor, title, body, contactRecipientId, recipientIds, rewardCredits, rewardItems });
          form.reset();
          await this.setSurface('messenger', { source: 'messenger' });
          this.render(false);
        });
      ...
      messengerRoot.querySelectorAll('[data-holonet-action="thread-action"]').forEach(el => {
        el.addEventListener('click', async (ev) => {
          ...
          await HolonetMessengerService.threadAction({ actor, threadId, action, recipientId });
          this.render(false);
        });
      });
      ...
      messengerRoot.querySelectorAll('form[data-holonet-action="transfer-credits"]').forEach(form => {
        form.addEventListener('submit', async (ev) => {
          ...
          const action = ev.submitter?.name === 'transferMode' && ev.submitter?.value ? ev.submitter.value : 'transfer-credits';
          await HolonetMessengerService.threadAction({ actor, threadId, action, recipientId, amount });
          form.reset();
          this.render(false);
        });
      });
