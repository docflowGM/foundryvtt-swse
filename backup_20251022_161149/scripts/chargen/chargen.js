  ...
  _bindAbilitiesUI(root) {
    ...
    function rollStandard() {
      ...
      for (let i=0;i<6;i++) {
        ...
        const rolls = (r.terms && r.terms[0] && r.terms[0].results) ? r.terms[0].results.map(x=>x.result).sort((a,b)=>a-b);
    ...
    function rollOrganic() {
      ...
      const rolls = (r.terms && r.terms[0] && r.terms[0].results) ? r.terms[0].results.map(x=>x.result).sort((a,b)=>b-a);
