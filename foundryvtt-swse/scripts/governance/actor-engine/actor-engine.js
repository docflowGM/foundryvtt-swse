...
export const ActorEngine = {
  ...
  _normalizeSkillStructureForContract(flat, actor) {
    ...
    for (const skillKey of skillKeys) {
      ...
      const props = ['trained', 'miscMod', 'focused', 'selectedAbility', 'favorite'];
      const currentSkill = actor?.system?.skills?.[skillKey] ?? {};

      for (const prop of props) {
        const path = `${basePath}.${prop}`;
        const defaults = {
          trained: currentSkill.trained ?? false,
          miscMod: Number.isFinite(Number(currentSkill.miscMod)) ? Number(currentSkill.miscMod) : 0,
          focused: currentSkill.focused ?? false,
          selectedAbility: typeof currentSkill.selectedAbility === 'string' ? currentSkill.selectedAbility : '',
          favorite: currentSkill.favorite ?? false
        };
